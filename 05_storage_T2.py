...
if simulate:
    ...
    while results.is_processing():
        ...
        # Plot results
        fig1.suptitle(
            f"Storage Cavity T2 for Fock state n=1 - LO = {storage_LO / u.GHz} GHz"
        )
        ax1[0].clear()
        ax1[1].clear()
        ax1[0].cla()
    ...
    try:
        ...
        print(
            f"Qubit detuning to update in the config: qubit_IF += {-qubit_detuning:.0f} Hz"
        )
        print(f"T2* = {qubit_T2:.0f} ns")
        plt.legend(
            (
                f"detuning = {-qubit_detuning / u.kHz:.3f} kHz",
                f"T2* = {qubit_T2:.0f} ns",
            )
        )
