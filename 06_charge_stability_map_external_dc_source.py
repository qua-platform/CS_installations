...
# Voltages in Volt
voltage_values_outer = np.linspace(-0.5, 0.5, n_points_outer)
voltage_values_slow = np.linspace(-0.5, 0.5, n_points_slow)
...
with program() as charge_stability_prog:
    ...
    with for_(k, 0, k < n_points_outer, k + 1):
...
if simulate:
    ...
    # Live plotting
    fig = plt.figure(figsize=(8 * n_points_outer, 6))
    interrupt_on_close(fig, job)  # Interrupts the job when closing the figure
    for k in range(n_points_outer):
        for i in range(n_points_slow):  # Loop over y-voltages
            ...
            for j in range(n_points_fast):  # Loop over x-voltages
                ...
                if k + 1 == n_points_outer and i + 1 == n_points_slow and j + 1 == n_points_fast:
                    break
                
                # Wait until the program reaches the 'pause' statement again, indicating that the QUA program is done
                wait_until_job_is_paused(job)
                print("    resumed & wait_until_job_is_paused")
            if k == 0 and i == 0:
                print(f"outer = {k}, slow = {i}: looping to outer = 0, slow = 1")
                # Get results from QUA program and initialize live plotting
                results = fetching_tool(job, data_list=["I", "Q", "iteration"], mode="live")
                print(f"   made the fetching tool")
            ...
            S = u.demod2volts(I_this + 1j * Q_this, reflectometry_readout_length)
    ...
    I, Q, iteration = results.fetch_all()
    I = I.reshape(n_points_outer, n_points_slow, n_points_fast)
    Q = Q.reshape(n_points_outer, n_points_slow, n_points_fast)
