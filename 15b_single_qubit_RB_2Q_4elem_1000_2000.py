...
# circuit_depth_min = 0
# target = 100_000
local_depth_max = 1000  # ~16k should work
...
duration_compensation_pulse_rb = 1_000_000  # duration_rb
...
with program() as PROG_RB:
    ...
    # n = [declare(int) for _ in range(len(target_qubits1))]   # QUA variable for the averages
    # I = [declare(fixed) for _ in range(len(target_qubits1))]  # QUA variable for the 'I' quadrature
    # Q = [declare(fixed) for _ in range(len(target_qubits1))]  # QUA variable for the 'Q' quadrature
    # P = [declare(bool) for _ in range(len(target_qubits1))]  # QUA variable for state discrimination
    ...
    with for_(m, 0, m < num_of_sequences, m + 1):  # QUA for_ loop over the random sequences
        ...
        for i, qb1 in enumerate(target_qubits1):
            ...
            wait(200 + 16015 * (i == 0) + 16 * (i == 1) + sequence_time_before1, qb1)
        ...
        for i, (qb1, qb2) in enumerate(zip(target_qubits1, target_qubits2)):
            ...
            wait(200 + 0 * (i == 0) - 16_000 * (i == 1) + sequence_time_before2, qb2)
