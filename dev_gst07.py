...
with program() as PROGRAM:
    ...
    with for_each_(n_shots, list_n_shots):
        ...
        with for_(mm, 0, mm < num_cicuits, mm + 1):
            ...
            with if_((prep_fiducical_idx >= 1) & (germ_idx >= 0) & (meas_fiducical_idx >= 1)):
                ...
                with for_(nn, 0, nn < n_shots, nn + 1):
                    ...
                    with strict_timing_():
                        wait(48, qbm)
                        with switch_(prep_fiducical_idx, unsafe=False):
                            with case_(1):
                                play("x90", qbm)
                            with case_(2):
                                play("y90", qbm)
                        ...
                        with switch_(germ_idx, unsafe=False):
                            with case_(0):
                                # wait(delay_twin, qbt)
...
if simulate:
    ...
    for _n_shots in list_n_shots:
        encoded_circuit_batch = dg[["index", "P_enc", "G_enc", "d_enc", "M_enc"]].values
