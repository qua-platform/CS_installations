...
frequencies = {
    "q1_rr": np.arange(-50e6, +50e6, 100e3),
...
with program() as PROGRAM:
    ...
    with for_(n, 0, n < n_avg, n + 1):  # QUA for_ loop for averaging
        with for_(*from_array(f, frequencies)):  # QUA for_ loop for sweeping the frequency
            ...
            update_frequency(resonator, f + RR_CONSTANTS[resonator]["IF"])
            ...
            wait(rr_reset_time  >> 2, resonator)
...
if __name__ == "__main__":
    ...
    if simulate:
        ...
        try:
            ...
            ax1 = plt.subplot(211)
            plt.plot((frequencies + RR_CONSTANTS[resonator]["IF"])/ u.MHz, R, ".")
            plt.ylabel(r"$R=\sqrt{I^2 + Q^2}$ [V]")
            plt.subplot(212, sharex=ax1)
            plt.plot((frequencies + RR_CONSTANTS[resonator]["IF"])/ u.MHz, signal.detrend(np.unwrap(phase)), ".")
