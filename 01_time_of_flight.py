...
import qm as qm_api
import numpy as np
from configuration import config, depletion_time, u, qop_ip, cluster_name
...
with qua.program() as raw_trace_prog:
    n = qua.declare(int)  # QUA variable for the averaging loop
    adc_st = qua.declare_stream(adc_trace=True)  # The stream to store the raw ADC trace
...
if simulate:
    ...
    time_of_flight_data['adc1'] = adc1
    time_of_flight_data['adc2'] = adc1
    time_of_flight_data['adc1_single_run'] = adc1_single_run
    time_of_flight_data['adc2_single_run'] = adc2_single_run

    data_handler.save_data(data=time_of_flight_data, name="time_of_flight")
