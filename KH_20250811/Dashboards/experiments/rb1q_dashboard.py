...
# --------------------------------------------------------------------
# 1. Data Loader for Randomized Benchmarking measurements with adaptive support
# --------------------------------------------------------------------
def load_rb_data(folder: str | Path) -> dict | None:
    """
    Load RB result files with adaptive support for state discrimination and I/Q modes
    ...
    - y_std: standard deviation/error for each qubit
    ...
    - use_state_discrimination: boolean indicating data type
    """
    ...
    # ── Determine data type (state discrimination vs I/Q) ──────────────
    has_state = "state" in ds_raw
    has_iq = "I" in ds_raw and "Q" in ds_raw
    use_state_discrimination = has_state and not has_iq
    
    # ── Get averaged data and calculate error bars ─────────────────────
    if use_state_discrimination:
        # State discrimination mode
        if "averaged_data" in ds_raw:
            y_data = ds_raw["averaged_data"].values  # (q, d)
        elif "state" in ds_raw:
            # Calculate average from raw state data
            state_data = ds_raw["state"]  # (q, sequences, d)
            y_data = state_data.mean(dim="nb_of_sequences").values
        else:
            raise KeyError("No state data found in dataset")
        
        # Calculate standard error
        if "state" in ds_raw and len(ds_raw["state"].dims) > 2:
            # Calculate standard deviation across sequences
            state_std = ds_raw["state"].std(dim="nb_of_sequences").values
            n_sequences = ds_raw["state"].shape[1] if len(ds_raw["state"].shape) > 2 else 1
            y_std = state_std / np.sqrt(n_sequences)  # Standard error of mean
        else:
            y_std = np.zeros_like(y_data) * 0.01  # Small default error
    else:
        # I/Q mode
        I_data = ds_raw["I"].values if "I" in ds_raw else None
        Q_data = ds_raw["Q"].values if "Q" in ds_raw else None
        
        if I_data is not None and Q_data is not None:
            # Calculate amplitude from I/Q data
            if len(I_data.shape) == 3:  # (q, sequences, d)
                # Average over sequences first
                I_avg = np.mean(I_data, axis=1)
                Q_avg = np.mean(Q_data, axis=1)
                y_data = np.sqrt(I_avg**2 + Q_avg**2)
                
                # Calculate error bars
                amp_per_seq = np.sqrt(I_data**2 + Q_data**2)
                amp_std = np.std(amp_per_seq, axis=1)
                n_sequences = I_data.shape[1]
                y_std = amp_std / np.sqrt(n_sequences)
            else:  # Already averaged
                y_data = np.sqrt(I_data**2 + Q_data**2)
                y_std = np.zeros_like(y_data) * 0.01
        elif "averaged_data" in ds_raw:
            y_data = ds_raw["averaged_data"].values
            y_std = np.zeros_like(y_data) * 0.01
        else:
            raise KeyError("No valid data found in dataset")
    ...
    # ── Success status and error metrics ───────────────────────────────────
    ...
    print(f"[load_rb_data] Data type: {'State discrimination' if use_state_discrimination else 'I/Q mode'}")
...
# --------------------------------------------------------------------
# 2. Plot Generation for Randomized Benchmarking measurements with error bars
# --------------------------------------------------------------------
def create_rb_plots(
    data,
    n_cols: int = 2,
    ...
    """
    Create RB plots with fits and error bars for multiple qubits.
    ...
    show_error_bars: if True, show error bars on data points
    """
    ...
    for plot_idx, orig_idx in enumerate(indices):
        ...
        y_error = data["y_std"][orig_idx, :] if "y_std" in data else None
        
        # Add raw data points with error bars
        if show_error_bars and y_error is not None:
            fig.add_trace(
                go.Scatter(
                    x=depths,
                    y=y_data,
        ...
        fit_valid = (
            ...
            not (fid_val >= MAX_VALID_FIDELITY)
        )
        ...
        fig.update_yaxes(
            ...
            row=r, col=c,
            showgrid=True,
            # range=[0, 1] if data["use_state_discrimination"] else None
        )
    ...
    # Layout
    data_type_str = "State Discrimination" if data["use_state_discrimination"] else "I/Q Mode"
    layout_args = dict(
        title=f"Single-qubit Randomized Benchmarking ({data_type_str})",
        legend=dict(
...
# --------------------------------------------------------------------
# 3. Create RB Performance Bar Plots (Fidelity and Error Rate)
# --------------------------------------------------------------------
def create_rb_performance_barplots(data):
    """
    Create side-by-side bar plots showing Gate Fidelity and Error Rate
    Similar to the matplotlib example in the reference image
    """
    if not data:
        return go.Figure()
    ...
    # Prepare data
    qubits = data["qubits"]
    error_per_gate = data["error_per_gate"]
    success = data["success"]
    
    # Calculate gate fidelities (not RB fidelity which is in percent)
    gate_fidelities = []
    for i in range(len(error_per_gate)):
        if success[i] and not np.isnan(error_per_gate[i]):
            gate_fidelities.append(1.0 - error_per_gate[i])
        else:
            gate_fidelities.append(np.nan)
    
    # Create subplots
    fig = subplots.make_subplots(
        rows=2, cols=1,
        subplot_titles=("Gate Fidelity by Qubit", "Gate Error Rate by Qubit"),
        horizontal_spacing=0.15
    )
    
    # Colors for fidelity plot
    fidelity_colors = []
    for fid in gate_fidelities:
        if np.isnan(fid):
            fidelity_colors.append("darkred")
        elif fid > 0.995:
            fidelity_colors.append("green")
        elif fid > 0.99:
            fidelity_colors.append("orange")
        else:
            fidelity_colors.append("red")
    
    # Colors for error rate plot
    error_colors = []
    for err in error_per_gate:
        if np.isnan(err):
            error_colors.append("darkred")
        elif err < 0.005:
            error_colors.append("green")
        elif err < 0.01:
            error_colors.append("orange")
        else:
            error_colors.append("red")
    
    # Add Gate Fidelity bars (left plot)
    fidelity_text = []
    for fid in gate_fidelities:
        if not np.isnan(fid):
            fidelity_text.append(f"{fid:.4f}")
        else:
            fidelity_text.append("Failed")
    
    fig.add_trace(
        go.Bar(
            x=qubits,
            y=[f if not np.isnan(f) else 0 for f in gate_fidelities],
            marker_color=fidelity_colors,
            text=fidelity_text,
    ...
    # Add Error Rate bars (right plot)
    error_text = []
    for err in error_per_gate:
        if not np.isnan(err):
            error_text.append(f"{err:.1e}")
        else:
            error_text.append("Failed")
    
    # Replace NaN with a small value for visualization
    display_errors = [err if not np.isnan(err) else 1e-10 for err in error_per_gate]
    ...
    # Update axes for Gate Fidelity plot
    fig.update_xaxes(
        title_text="",
        tickangle=-45,
        showgrid=False,
    ...
    fig.update_yaxes(
        ...
        showline=True,
        linecolor="black",
        range=[0.98, 1.0] if all(f > 0.98 or np.isnan(f) for f in gate_fidelities) else [0, 1],
        row=1, col=1
    )
    
    # Update axes for Error Rate plot (log scale)
    fig.update_xaxes(
        title_text="",
        tickangle=-45,
        showgrid=False,
...
# --------------------------------------------------------------------
# 4. Summary Table for RB measurements
# --------------------------------------------------------------------
...
# --------------------------------------------------------------------
# 5. Layout + Callbacks for RB measurements with bar plot
# --------------------------------------------------------------------
def create_rb_layout(folder):
    """Create the complete layout for RB visualization with performance bar plots"""
    ...
    init_fig = create_rb_plots(data, n_cols=2, fig_height=default_height, show_error_bars=True)
    init_performance_fig = create_rb_performance_barplots(data)
            ...
            dbc.AccordionItem(
                ...
                children=[
                    dbc.Row([
                        ...
                        dbc.Col(html.Div([
                            html.Label("Show Error Bars"),
                            dcc.Checklist(
                                id={"type": "rb-errorbars", "index": uid},
                                options=[{"label": "Error Bars", "value": "show"}],
                                value=["show"],
                                inline=True
                            )
                        ]), width=2),
    ...
    # Data mode alert
    data_mode_alert = dbc.Alert(
        f"Data Mode: {'State Discrimination' if data['use_state_discrimination'] else 'I/Q Quadratures'}",
        color="info",
        className="mb-3"
    )
...
"""
        ...
        children=[
            ...
            html.Div(
                ...
                children=[
                    summary_content,
                    html.Hr(),
                    html.H6("Performance Analysis"),
                    html.Div([
                        dcc.Graph(
                            id={"type": "rb-performance-plots", "index": uid},
                            figure=init_performance_fig,
    ...
    # Main layout with performance bar plots at bottom
    return html.Div([
...
def register_rb_callbacks(app):
    ...
    def update_rb_plot(filter_mode, errorbars, n_cols, fig_w, fig_h, store):
        """Update RB plot based on user inputs"""
        data = load_rb_data(store["folder"])
        show_only_successful = (filter_mode == "success")
        show_error_bars = "show" in errorbars if errorbars else False
        ...
        performance_fig = create_rb_performance_barplots(data)
        summary_table = create_rb_summary_table(data)
        return fig, performance_fig, summary_table
