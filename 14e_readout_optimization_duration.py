...
with program() as PROGRAM:
    ...
    with stream_processing():
        ...
        for rr in resonators:
            Ig_st.buffer(number_of_divisions).save_all(f"I_g_{rr}")
            Qg_st.buffer(number_of_divisions).save_all(f"Q_g_{rr}")
            Ie_st.buffer(number_of_divisions).save_all(f"I_e_{rr}")
            Qe_st.buffer(number_of_divisions).save_all(f"Q_e_{rr}")
        ...
        try:
            ...
            while results.is_processing():
                ...
                for ind, (qb, rr) in enumerate(zip(qubits, resonators)):
                    ...
                    plt.clf()
                    plt.plot(pulse_duration, iq_blobs_result)
                    plt.title(f"Qb - {qb}")
