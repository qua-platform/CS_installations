...
with program() as calib_delays:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        wait(
            initial_delay_cycles, "OpticalTrigger", "EOM", "AOM2"
        ...
        OpticalTrigger.play(
            "Laser_ON",  duration=laser_len_cycles
        )  # Cycle through each of these for various delays
        AOM2.play("AOM2_ON", duration= laser_len_cycles)


        wait(
            initial_delay_cycles + (laser_len_cycles - var_len_cycles) // 2, "AOM1"
        )  # delay the pulse until
        AOM1.play("AOM1_ON", duration=var_len_cycles)  # play microwave pulse
