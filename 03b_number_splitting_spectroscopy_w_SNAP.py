...
with program() as number_splitting_spectroscopy:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(*from_array(df, dfs)):
            ...
            play(
                "x360_long", "qubit"
            )  # play a selective 2pi pulse at qubit frequency that corresponds to n=0
            align("qubit", "storage")
            play("beta2", "storage")
...
if simulate:
    ...
    results = fetching_tool(
        job, data_list=["I", "state", "Q", "iteration"], mode="live"
    )
    # Live plotting
    ...
    while results.is_processing():
        ...
        # Plot results
        fig1.suptitle(
            f"Number Splitting Spectroscopy at Fock state n=1 - LO = {storage_LO / u.GHz} GHz"
        )
        ax1[0].clear()
        ax1[1].clear()
        ax1[0].cla()
