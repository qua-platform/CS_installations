...
for i, rr in enumerate(resonators):
    ...
    # I_avg = fetched_data[6 * i + 2]
    # Q_avg = fetched_data[6 * i + 3]
    s1 = fetched_data[4 * i + 2]
    s2 = fetched_data[4 * i + 3]
    
    # assert np.array_equal(I, I_avg)
    # assert np.array_equal(Q, Q_avg)
    assert len(I) == len(frequencies)
    assert len(Q) == len(frequencies)
    assert np.array_equal(I, np.ones(len(frequencies)))
    assert np.array_equal(Q, -np.ones(len(frequencies)))
