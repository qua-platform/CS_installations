...
amp_scalings = np.arange(0.96, 1.04, 0.004)
...
for idx, amp in enumerate(amp_scalings):
    hcr = CRHamiltonian(A_c, amp * A_t, phi_c, phi_t)
    hcr.print_params()
...
plot_cr_duration_vs_scan_param(
    ...
    ts,
    amp_scalings,
    "cancel drive amplitude scaling",
    axss,
)

# Plot the estimated interaction coefficients
fig_summary = plot_interaction_coeffs(coeffs, amp_scalings, xlabel="cancel drive amplitude scaling")
plt.show()
