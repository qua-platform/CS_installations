...
phase_scalings = np.arange(0.35, 0.38, 0.001)
...
for idx, ph in enumerate(phase_scalings):
    hcr = CRHamiltonian(A_c, A_t, ph * two_pi, phi_t)
    hcr.print_params()
...
for idx, ph in enumerate(phase_scalings):
    ...
    try:
        crht = CRHamiltonianTomographyAnalysis(
            ts=ts,
            data=target_data[idx, ...],  # target data: len(phases) x len(t_vec_cycle) x 3 x 2
        )
        crht.fit_params()
...
fig_summary = plot_interaction_coeffs(coeffs, phase_scalings, xlabel="cr drive phase")
plt.show()
