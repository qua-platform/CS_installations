...
Before proceeding to the next node:
    - Update the readout frequency, labeled as "resonator_IF_q1", in the configuration.
    - Adjust the readout amplitude, labeled as "readout_amp_q1", in the configuration.
"""
...
# The frequency sweep around the resonator frequency "resonator_IF_q1"
span = 10 * u.MHz
df = 100 * u.kHz
...
if simulate:
    ...
    while results.is_processing():
        ...
        plt.subplot(211)
        plt.suptitle(f"Resonator spectroscopy - LO = {resonator_LO / u.GHz} GHz & IF = {resonator_IF_q1 / u.MHz} MHz")
        plt.cla()
        plt.title(r"$R=\sqrt{I^2 + Q^2}$ (normalized)")
        plt.pcolor(amplitudes * readout_amp_q1, dfs / u.MHz, R)
        plt.ylabel("Readout detuning [MHz]")
        plt.subplot(212)
        plt.cla()
        plt.title("Phase")
        plt.pcolor(amplitudes * readout_amp_q1, dfs / u.MHz, signal.detrend(np.unwrap(phase)))
        plt.ylabel("Readout detuning [MHz]")
