...
@app.callback(
    ...
    [State("custom-folder-input", "value"),
     ...
     State("folder-remove-trigger", "data")],
)
def manage_custom_folders(add_clicks, clear_clicks, remove_clicks, new_path, current_folders, trigger):
    ctx = dash.callback_context
    if not ctx.triggered:
        return current_folders or [], "", "", trigger
    
    button_id = ctx.triggered[0]["prop_id"]
    
    # Handle individual folder removal
    if "remove-folder-btn" in button_id:
        try:
            import json
            prop_dict = json.loads(button_id.split(".")[0])
            folder_to_remove = prop_dict["index"]
            updated_folders = [f for f in (current_folders or []) if f != folder_to_remove]
            return updated_folders, dbc.Alert(f"Removed: {folder_to_remove}", color="warning", dismissable=True, duration=3000), "", trigger + 1
        except:
            pass
    
    # Handle clear all
    if "clear-folders-button" in button_id:
        return [], dbc.Alert("All custom folders cleared!", color="warning", dismissable=True, duration=3000), "", trigger + 1
    ...
    if "add-folder-button" in button_id and new_path:
        ...
        if not os.path.exists(new_path):
            return current_folders or [], dbc.Alert(f"Path does not exist: {new_path}", color="danger", dismissable=True), new_path, trigger
        
        if not os.path.isdir(new_path):
            return current_folders or [], dbc.Alert(f"Path is not a directory: {new_path}", color="danger", dismissable=True), new_path, trigger
        
        # Check if already in list
        if new_path in (current_folders or []):
            return current_folders, dbc.Alert(f"Path already added: {new_path}", color="warning", dismissable=True), "", trigger
        
        # Add to list
        updated_folders = (current_folders or []) + [new_path]
        return updated_folders, dbc.Alert(f"Successfully added: {new_path}", color="success", dismissable=True, duration=3000), "", trigger + 1
    
    return current_folders or [], "", "", trigger

# Callback to display watched folders with remove buttons
@app.callback(
    Output("watched-folders-list", "children"),
    [Input("additional-folders", "data"),
     Input("folder-remove-trigger", "data")],
)
def display_watched_folders(additional_folders, _):
    ...
    for i, folder in enumerate(all_folders):
        ...
        icon = "✅" if exists else "❌"
        label = "Default" if i == 0 else f"Custom {i}"
        
        # Count experiments in this folder
        exp_count = 0
        if exists:
            folder_exps = find_experiments(folder)
            exp_count = sum(len(exps) for exps in folder_exps.values())
        
        # Create folder item with optional remove button
        if i == 0:  # Default folder - no remove button
            folder_items.append(
                dbc.ListGroupItem([
                    html.Div([
                        html.Div([
                            ...
                            html.Span(f" ({exp_count} experiments)" if exp_count > 0 else " (no experiments)", 
                                    className="text-muted ms-2", style={"fontSize": "11px"}),
                        ], className="flex-grow-1"),
                        html.Div([
                            dbc.Badge("Default - Cannot Remove", color="secondary", className="ms-2", pill=True)
                        ])
                    ], className="d-flex align-items-center justify-content-between")
                ], className="py-2", color="light" if i == 0 else None)
            )
        else:  # Custom folders - with remove button
            folder_items.append(
                dbc.ListGroupItem([
                    html.Div([
                        html.Div([
    ...
    summary = dbc.Alert(
        f"📊 Watching {total_folders} folder{'s' if total_folders != 1 else ''}: "
...
# Modified polling callback to check multiple folders
@app.callback(
...
def poll_folders(_, additional_folders, cur):
    # Combine default path with additional folders
    all_paths = [EXPERIMENT_BASE_PATH] + (additional_folders or [])
    
    # Find experiments from all paths
    new = find_experiments_multiple_paths(all_paths)
    
    # Count total experiments
    total_exp_count = sum(len(exps) for exps in new.values())
    folder_count = len([p for p in all_paths if os.path.exists(p)])
    
    count_info = f"Found {total_exp_count} experiments across {folder_count} accessible folder{'s' if folder_count != 1 else ''}"
