...
def active_reset(threshold, max_tries=1, Ig=None):
    ...
    with while_((Ig > threshold) & (counter < max_tries)):
        ...
        play("pi", "q1_xy", condition=(Ig > threshold))
...
class qubit_frequency_tracking:
    def __init__(self, qubit, rr, f_res, ge_threshold_q1, frame_rotation_flag=False):
        """Frequency tracking class
        ...
        :param float ge_threshold_q1: Threshold to discriminate between ground and excited (with single shot readout)
        ...
        self.ge_threshold_q1 = ge_threshold_q1
    ...
    def time_domain_ramsey_full_sweep(self, n_avg, f_det, tau_vec, correct=False):
        ...
        with for_(self.n, 0, self.n < n_avg, self.n + 1):
            with for_(*from_array(self.tau, tau_vec)):
                ...
                assign(self.res, self.I > self.ge_threshold_q1)
                ####################################################################################################
