...
class ChangeOscillatorFrequencyRequestUpdateComponentSelection(betterproto.Enum):
    both = 0
    upconverter = 1
    downconverter = 2
...
class GetElementCorrectionRequest(betterproto.Message):
    ...
    def __post_init__(self) -> None:
        super().__post_init__()
        if self.is_set("correction"):
            warnings.warn(
                "GetElementCorrectionRequest.correction is deprecated",
                DeprecationWarning,
            )
...
@dataclass(eq=False, repr=False)
class ChangeOscillatorFrequencyRequest(betterproto.Message):
    job_id: str = betterproto.string_field(1)
    element_name: str = betterproto.string_field(2)
    new_frequency_hz: float = betterproto.double_field(3)
    update_component: "ChangeOscillatorFrequencyRequestUpdateComponentSelection" = (
        betterproto.enum_field(4)
    )


@dataclass(eq=False, repr=False)
class ChangeOscillatorFrequencyResponse(betterproto.Message):
    job_manager_response_header: "JobManagerResponseHeader" = betterproto.message_field(
        1
    )
...
class JobManagerServiceStub(betterproto.ServiceStub):
    ...
    async def update_oscillator_frequency(
        self,
        change_oscillator_frequency_request: "ChangeOscillatorFrequencyRequest",
        *,
        timeout: Optional[float] = None,
        deadline: Optional["Deadline"] = None,
        metadata: Optional["MetadataLike"] = None
    ) -> "ChangeOscillatorFrequencyResponse":
        return await self._unary_unary(
            "/qm.grpc.job_manager.JobManagerService/UpdateOscillatorFrequency",
            change_oscillator_frequency_request,
            ChangeOscillatorFrequencyResponse,
            timeout=timeout,
            deadline=deadline,
            metadata=metadata,
...
class JobManagerServiceBase(ServiceBase):
    ...
    async def update_oscillator_frequency(
        self, change_oscillator_frequency_request: "ChangeOscillatorFrequencyRequest"
    ) -> "ChangeOscillatorFrequencyResponse":
        raise grpclib.GRPCError(grpclib.const.Status.UNIMPLEMENTED)
    ...
    async def __rpc_update_oscillator_frequency(
        self,
        stream: "grpclib.server.Stream[ChangeOscillatorFrequencyRequest, ChangeOscillatorFrequencyResponse]",
    ) -> None:
        request = await stream.recv_message()
        response = await self.update_oscillator_frequency(request)
        await stream.send_message(response)
    ...
    def __mapping__(self) -> Dict[str, grpclib.const.Handler]:
        return {
            ...
            "/qm.grpc.job_manager.JobManagerService/UpdateOscillatorFrequency": grpclib.const.Handler(
                self.__rpc_update_oscillator_frequency,
                grpclib.const.Cardinality.UNARY_UNARY,
                ChangeOscillatorFrequencyRequest,
                ChangeOscillatorFrequencyResponse,
            ),
        }
