...
res_LO = 7.0 * u.GHz
res_IF = 60 * u.MHz
res_power = -11  # power in dBm (steps of 3 dB)
...
#                  Config                   #
#############################################
globals()["out{}_band".format(res_out_port)] = get_band(res_LO + res_IF)
globals()["out{}_up1_LO".format(res_out_port)] = res_LO
if res_out_port == 1:
    out1_band = in1_band = get_band(res_LO + res_IF)
    out1_up1_LO = in1_LO = res_LO
elif res_out_port in [2, 3]:
    out2_band = out3_band = get_band(res_LO + res_IF)
    out2_up1_LO = out3_up1_LO = res_LO
elif res_out_port in [4,5]:
    out4_band = out5_band = get_band(res_LO + res_IF)
    out4_up1_LO = out5_up1_LO = res_LO
elif res_out_port in [6,7]:
    out6_band = out7_band = get_band(res_LO + res_IF)
    out6_up1_LO = out7_up1_LO = res_LO
elif res_out_port == 8:
    out8_band = in2_band = get_band(res_LO + res_IF)
    out8_up1_LO = in2_LO = res_LO
globals()["out{}_power".format(res_out_port)] = res_power
globals()["in{}_gain_db".format(res_in_port)] = res_in_gain_db
if res_in_port == 1:
    out1_band = in1_band = globals()["out{}_band".format(res_out_port)]
    out1_up1_LO = in1_LO = res_LO
elif res_in_port == 2:
    out8_band = in2_band = globals()["out{}_band".format(res_out_port)]
    out8_up1_LO = in2_LO = res_LO
...
config = {
    ...
    "elements": {
        "resonator": {
            "MWInput": {
                "port": (con, res_out_fem, res_out_port),
                "upconverter": 1,
            },
            "intermediate_frequency": res_IF,
            "operations": {
                "cw": "const_pulse",
                "readout": "readout_pulse",
            },
            "MWOutput": {
                "port": (con, res_in_fem, res_in_port),
            },
