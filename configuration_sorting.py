...
constant_pulse_amplitude = (
    0.45 / max_number_of_tweezers
)  # Must be < 0.49/max_number_of_tweezers
...
column_if = [
    int(column_if_first_site + column_spacing * i) for i in range(number_of_columns)
]
# --> Row frequencies
row_spacing = 0.76e6  # in Hz
row_selector_if = 68.6e6  # in Hz
row_frequencies_list = [
    row_selector_if + row_spacing * x for x in range(number_of_rows)
]
...
config = {
    ...
    "controllers": {
        "con1": {
            ...
            "analog_outputs": {
                ...
                column_channel: {
                    "offset": column_selector_voltage_offset
                },  # Column AOD tone
    ...
    "elements": {
        ...
        "fpga": {
            "singleInput": {"port": ("con1", 7)},  # Fake output port for measurement
    ...
    "waveforms": {
        "blackman_up_wf": {
            "type": "arbitrary",
            "samples": blackman(
                blackman_pulse_length / (1e9 / sampling_rate), 0, blackman_amplitude
            ),
        ...
        "blackman_down_wf": {
            "type": "arbitrary",
            "samples": blackman(
                blackman_pulse_length / (1e9 / sampling_rate), blackman_amplitude, 0
            ),
            "sampling_rate": sampling_rate,
        },
        "constant_wf": {
