...
cr_gaussian_rise_fall_pad_len = (
    0 if cr_gaussian_rise_fall_len % 4 == 0 else (4 - cr_gaussian_rise_fall_len % 4)
)
cr_square_phase = 0.0
...
ts_cycles = (2 * cr_gaussian_rise_fall_len // 4) + np.arange(
    t_min_flattop, t_max_flattop, dt
)  # in clock cylcle = 4ns
...
def get_flattop_gaussian(
    cr_amp, cr_flattop_len, cr_rise_fall_len, cr_rise_fall_pad_len
):
...
with program() as PROGRAM:
    ...
    with stream_processing():
        ...
        for ind, rr in enumerate(resonators):
            I_st[ind].buffer(2).buffer(3).buffer(len(ts_cycles)).average().save(
                f"I_{rr}"
            )
            Q_st[ind].buffer(2).buffer(3).buffer(len(ts_cycles)).average().save(
                f"Q_{rr}"
            )
            state_st[ind].boolean_to_int().buffer(2).buffer(3).buffer(
                len(ts_cycles)
            ).average().save(f"state_{rr}")
