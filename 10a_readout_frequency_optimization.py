...
with program() as ro_freq_opt:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(*from_array(df, dfs)):
            ...
            update_frequency(res_key, df + IF_RR)
...
if simulate:
    ...
    while results.is_processing():
        ...
        plt.title(f"Readout frequency optimization around {IF_RR / u.MHz} MHz")
        plt.xlabel("Readout frequency detuning [MHz]")
        plt.ylabel("SNR")
        plt.grid("on")
        plt.pause(0.1)
    print(f"The optimal readout frequency is {dfs[np.argmax(SNR)] + IF_RR} Hz (SNR={max(SNR)})")
