...
Next steps before going to the next node:
    - Update the readout frequency (resonator_IF_q1) in the configuration.
"""
...
if simulate:
    ...
    while results.is_processing():
        ...
        plt.title(f"Readout frequency optimization around {resonator_IF_q1 / u.MHz} MHz")
        plt.xlabel("Readout frequency detuning [MHz]")
        plt.ylabel("SNR")
        plt.grid("on")
        plt.pause(0.1)
    print(f"The optimal readout frequency is {dfs[np.argmax(SNR)] + resonator_IF_q1} Hz (SNR={max(SNR)})")
