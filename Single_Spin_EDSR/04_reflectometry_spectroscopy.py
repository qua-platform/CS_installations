...
# The QUA program #
###################
nb_of_tank_circuits=8
with program() as reflectometry_spectro:
    df = declare(int)  # QUA variable for the frequency sweep
    n = declare(int)  # QUA variable for the averaging loop
    I = [declare(fixed) for _ in range(nb_of_tank_circuits)]  # QUA variable for the measured 'I' quadrature
    Q = [declare(fixed) for _ in range(nb_of_tank_circuits)]  # QUA variable for the measured 'Q' quadrature
    I_st = [declare_stream() for _ in range(nb_of_tank_circuits)]  # Stream for the 'I' quadrature
    Q_st = [declare_stream() for _ in range(nb_of_tank_circuits)]  # Stream for the 'Q' quadrature
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(*from_array(df, frequencies)):
            for i in range(nb_of_tank_circuits):
                ...
                update_frequency(f"tank_circuit{i+1}", df + readout_frequencies[i])
                ...
                measure("readout", f"tank_circuit{i+1}", demod.full("cos", I[i], "out1"))
                save(I[i], I_st[i])
                save(Q[i], Q_st[i])
                ...
                wait(1_000 * u.ns, f"tank_circuit{i+1}")  # in ns
        save(n, n_st)
...
if simulate:
    ...
    data_list = ["iteration"] + np.concatenate([[f"I{i+1}", f"Q{i+1}"] for i in range(nb_of_tank_circuits)]).tolist()
    ...
    while results.is_processing():
        ...
        for i in range(nb_of_tank_circuits):
            ...
            S = u.demod2volts(IQ_data[f"tank_circuit{i+1}"]["I"] + 1j * IQ_data[f"tank_circuit{i+1}"]["Q"], reflectometry_readout_length * (sampling_rate/1e9), single_demod=True)
            IQ_data[f"tank_circuit{i + 1}"]["R"] = np.abs(S)
            IQ_data[f"tank_circuit{i + 1}"]["phase"] = np.angle(S)
        # Progress bar
        progress_counter(iteration, n_avg, start_time=results.get_start_time())

        rows = int(np.ceil(np.sqrt(nb_of_tank_circuits)))
        cols = int(np.ceil(nb_of_tank_circuits / rows))
        fig, axes = plt.subplots(rows, cols, figsize=(15, 10))
        fig.suptitle("Amplitude")
        for i in range(nb_of_tank_circuits):
            ax = axes[i // cols, i % cols]
            ax.plot((frequencies + readout_frequencies[i]) / u.MHz, IQ_data[f"tank_circuit{i + 1}"]["R"] / u.mV)
        ...
        for i in range(nb_of_tank_circuits):
            ax = axes[i // cols, i % cols]
            ax.plot((frequencies + readout_frequencies[i]) / u.MHz, signal.detrend(np.unwrap(IQ_data[f"tank_circuit{i + 1}"]["phase"])))
            ax.set_xlabel("Frequency [MHz]")
