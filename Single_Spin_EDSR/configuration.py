...
#              OPX PARAMETERS               #
#############################################
sampling_rate = int(2e9)  # or, int(2e9)
...
reflectometry_readout_amp = 50 * u.mV
rotation_angle = np.pi/4 - 270*np.pi/180

# Time of flight
time_of_flight = 28 + 264
            ...
            "fems": {
                ...
                lf_fem: {
                    ...
                    "analog_inputs": {
                        ...
                        2: {"offset": 0.0, "gain_db": 0, "sampling_rate": 2e9},  # DC readout input
                    },
                },
            },
        }
...
nb_of_tank_circuits = 2
readout_frequencies = np.array([10, 115, 350, 545, 660, 233, 752, 800]) * u.MHz
# readout_frequencies = np.array([50]*nb_of_tank_circuits) * u.MHz
for i in range(nb_of_tank_circuits):
    config["elements"][f"tank_circuit{i+1}"] = {
            "singleInput": {
                "port": (con, lf_fem, 7),
            },
            "intermediate_frequency": readout_frequencies[i],
            "operations": {
                "readout": "reflectometry_readout_pulse",
            },
            "outputs": {
