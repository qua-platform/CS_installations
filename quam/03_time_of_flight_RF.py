...
if simulate:
    ...
    delay = int(np.round(delay / 2 / 4) * 4)  # Find the closest multiple integer of 4ns
    ...
    yl = plt.ylim()
    plt.plot(xl, adc1_mean * np.ones(2), "k--")
    plt.plot(2 * delay * np.ones(2), yl, "k--")
    plt.xlabel("Time [ns]")
    plt.legend()
    plt.grid("all")
    plt.tight_layout()
    ...
    for i, q in enumerate(machine.qubits):
        resonator = machine.qubits[q].resonator
        resonator.time_of_flight += delay
    resonator.opx_input.offset -= adc1_mean
