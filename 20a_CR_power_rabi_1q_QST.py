# %%
"""
        Cross-Resonance Power Rabi with single-qubit Quantum State Tomography
...
the target qubit; the cross-resonance pulse has a variable amplitude. In the second sequence, we initialize the control
...
in |g> in both sequences when CR amplitude -> zero. At the end of both sequences we perform single-qubit Quantum
...
with program() as cr_time_rabi_one_qst:
    ...
    with stream_processing():
        ...
        for r, rr in enumerate(resonators):
            I_st[r].buffer(2).buffer(3).buffer(len(a_vec)).average().save(f"I{rr}")
            Q_st[r].buffer(2).buffer(3).buffer(len(a_vec)).average().save(f"Q{rr}")
        ...
        for i, (axs, bss) in enumerate(zip(axss, ["X", "y", "z"])):
            for ax, q in zip(axs, ["c", "t"]):
                ...
                for j, st in enumerate(["0", "1"]):
                    ax.plot(a_vec, I[:, i, j], label=[f"|{st}>"])
                ax.legend(["0", "1"])
                ax.set_title(f"Q_{q}") if i == 0 else None
                ax.set_xlabel("cr amplitudes") if i == 2 else None
