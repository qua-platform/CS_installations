...
with program() as EOM_sweep:
    ...
    with for_(b, 0, b <= b_count, b + 1):
        ...
        with for_(*from_array(f, frequencies)):
            ...
            update_frequency("EOM", f + EOM.intermediate_frequency)

            with for_(n, 0, n < n_avg, n + 1):

                AOM1.play("AOM1_ON", duration=pulse_len)
                AOM2.play("AOM2_ON", duration=pulse_len)
                OpticalTrigger.play("Laser_ON", duration=pulse_len)
                EOM.play("const_pulse",  duration=pulse_len)
