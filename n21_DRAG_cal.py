...
import qm as qm_api
import numpy as np
from configuration import config, qop_ip, cluster_name, u, thermalization_time, drag_coef, ge_threshold, readout_len
...
with qua.program() as drag:
    ...
    a = qua.declare(qua.fixed)  # QUA variable for the DRAG coefficient pre-factor
    it = qua.declare(int)  # QUA variable for the number of qubit pulses
    pulses = qua.declare(int)  # QUA variable for counting the qubit pulses
    ...
    with qua.for_(n, 0, n < n_avg, n + 1):  # QUA for_ loop for averaging
        with qua.for_(*from_array(a, amps)):  # QUA for_ loop for sweeping the pulse amplitude
            with qua.for_(*from_array(it, iters)):  # QUA for_ loop for sweeping the number of pulses
                # Loop for error amplification (perform many qubit pulses with varying DRAG coefficients)
                with qua.for_(pulses, iter_min, pulses <= it, pulses + d):
                    qua.play("x180" * qua.amp(1, 0, 0, a), "qubit")
                    qua.play("x180" * qua.amp(-1, 0, 0, -a), "qubit")
