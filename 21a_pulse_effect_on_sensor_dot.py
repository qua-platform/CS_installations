...
with program() as PROG:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_each_(wt, wait_time_clock_cycles):
    ...
    with stream_processing():
        ...
        I_st.buffer(number_of_divisions).buffer(len(wait_time_array)).average().save(
            "I"
        )
        Q_st.buffer(number_of_divisions).buffer(len(wait_time_array)).average().save(
            "Q"
        )
