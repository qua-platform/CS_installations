        ...
        for i, (axs, bss) in enumerate(zip(axss, ["X", "Y", "Z"])):
            for j, stc in enumerate(["g", "e"]):
                ...
                if node.parameters.use_state_discrimination:
                    axs[0].plot(ds_sliced.pulse_duration.data, ds_sliced[f"state_c_{qp.name}"].data, label=[f"qc=|{stc}>"])
                    axs[1].plot(ds_sliced.pulse_duration.data, ds_sliced[f"state_t_{qp.name}"].data, label=[f"qt=|{stc}>"])
                    axs[0].set_ylabel(f"State in <{bss}>")
                else:
                    axs[0].plot(ds_sliced.pulse_duration.data, ds_sliced[f"I_c_{qp.name}"].data, label=[f"qc=|{stc}>"])
                    axs[1].plot(ds_sliced.pulse_duration.data, ds_sliced[f"I_t_{qp.name}"].data, label=[f"qt=|{stc}>"])
