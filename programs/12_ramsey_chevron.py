...
with program() as ramsey_chevron_multiplexed:
    ...
    with stream_processing():
        ...
        for j in range(len(qub_key_subset)):
            I_st[j].buffer(len(qub_detune_sweep_dfs)).buffer(len(taus_cycles)).average().save("I_"+str(j))
            Q_st[j].buffer(len(qub_detune_sweep_dfs)).buffer(len(taus_cycles)).average().save("Q_"+str(j))
