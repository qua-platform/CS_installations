...
with program() as rb:
    ...
    cayley = [declare(int, value=c1_table.flatten().tolist()) for _ in range(2)]
    ...
    with for_(n, 0, n < n_avg, n + 1):  # Averaging loop
        ...
        with strict_timing_():
            ...
            with for_(i[0], 0, i[0] < max_circuit_depth, i[0] + 1):
                assign(step[0], rand[0].rand_int(24))
                assign(current_state[0], cayley[0][current_state[0] * 24 + step[0]])
                assign(sequence[0][i[0]], step[0])
                # assign(inv_gate[0][i], inv_list[0][current_state])

            # play("square_y180", qb)
            play_sequence(sequence[0], max_circuit_depth, qubit)
            # play_clifford(step[0], qubit)

            # wait((duration_init >> 2), qb)
            # play_sequence(sequence_list1, depth, qb=qb)
