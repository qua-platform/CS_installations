...
rr_freq = np.array([5.9, 6.05, 6.2, 6.35, 6.5]) * u.GHz
rr_LO = 6.25 * u.GHz
...
xy_freq = np.array([4.4, 4.55, 4.7, 4.85, 5.0]) * u.GHz
xy_LO = 4.75 * u.GHz
...
anharmonicity = np.array([150, 200, 175, 310, 214]) * u.MHz
...
for k, q in enumerate(machine.qubits):
    ...
    machine.qubits[q].xy.operations["const"] = SquarePulse(length = 1 * u.us, amplitude = xy_amplitude)
