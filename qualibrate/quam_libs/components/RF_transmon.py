...
from quam.components.channels import SingleChannel, Pulse
from quam import QuamComponent
from quam.utils.qua_types import ScalarFloat, ScalarInt, ScalarBool
...
from qm.qua import align, wait, ChirpType, StreamType
import numpy as np
...
@quam_dataclass
class RF_Transmon(QuamComponent):
    """
    ...
    I: SingleChannel = None
    Q: SingleChannel = None
    z: FluxLine = None
    ...
    def __matmul__(self, other):
        if not isinstance(other, RF_Transmon):
            raise ValueError(
    ...
    def align(self, other = None):
        channels = [self.I.name, self.Q.name, self.resonator.name, self.z.name]

        if other is not None:
            channels += [other.I.name, other.Q.name, other.resonator.name, other.z.name]

        align(*channels)

    def wait(self, duration):
        wait(duration, self.I.name, self.Q.name, self.z.name, self.resonator.name)
    ...
    def xy_play(self,
        ...
        self.Q.frame_rotation_2pi(0.25)
        align(self.I.name, self.Q.name)
        self.I.play(
            operation,
        ...
        reset_frame(self.Q.name)


    def xy_wait(self, duration):
        wait(duration, self.I.name, self.Q.name)

    def xy_update_frequency(self,
