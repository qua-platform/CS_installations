...
from quam_libs.quam_builder.pulses import add_default_transmon_pulses, add_default_transmon_pair_pulses, add_default_rf_transmon_pulses
...
from quam_libs.quam_builder.transmons.add_rf_I_qubit_component import add_rf_I_qubit_component
from quam_libs.quam_builder.transmons.add_rf_Q_qubit_component import add_rf_Q_qubit_component
from qualang_tools.wirer.connectivity.wiring_spec import WiringLineType
from quam_libs.components import OPXPlusQuAM, FEMQuAM, QuAM, Transmon, RF_Transmon
...
def add_transmons(machine: QuAM):
    for element_type, wiring_by_element in machine.wiring.items():
        ...
        if element_type == "rf_qubits":
            for qubit_id, wiring_by_line_type in wiring_by_element.items():
                transmon = RF_Transmon(id=qubit_id)
                for line_type, ports in wiring_by_line_type.items():
                    machine.rf_qubits[qubit_id] = transmon
                    wiring_path = f"#/wiring/{element_type}/{qubit_id}/{line_type}"
                    if line_type == WiringLineType.RESONATOR.value:
                        add_transmon_resonator_component(transmon, wiring_path, ports, machine)
                    elif line_type == WiringLineType.DRIVE.value:
                        ...
                        add_transmon_flux_component(transmon, wiring_path, ports)
                    elif line_type == WiringLineType.DRIVE_I.value:
                        add_rf_I_qubit_component(transmon, wiring_path, ports)
                    elif line_type == WiringLineType.DRIVE_Q.value:
                        add_rf_Q_qubit_component(transmon, wiring_path, ports)
                    else:
                        raise ValueError(f"Unknown line type: {line_type}")
                machine.active_qubit_names.append(transmon.name)
...
def add_pulses(machine: QuAM):
    ...
    if hasattr(machine, "rf_qubits"):
        for rf_qubit in machine.rf_qubits.values():
            add_default_rf_transmon_pulses(rf_qubit)
