...
def add_default_rf_transmon_pulses(transmon: Transmon):
    # TODO: Make gates amplitude a reference to x180 amplitude
    if transmon.I is not None:
        # Cosine pulse
        transmon.I.operations["x180_Cosine"] = pulses.DragCosinePulse_I(
            amplitude=0.1,
            alpha=0.0,
        ...
        transmon.I.operations["x90_DragCosine"] = pulses.DragCosinePulse_I(
            amplitude=0.1 / 2,
            alpha="#../x180_DragCosine/alpha",
            anharmonicity="#../x180_DragCosine/anharmonicity",
            axis_angle=0,
            length="#../x180_DragCosine/length",
            detuning="#../x180_DragCosine/length",
            digital_marker="ON",
        )

        # Square pulses
        transmon.I.operations["x180_Square"] = pulses.SquarePulse(
            amplitude=0.25, length=32, digital_marker="ON"
        )
        transmon.I.operations["x90_Square"] = pulses.SquarePulse(
            amplitude=0.25 / 2, length="#../x180_Square/length", digital_marker="ON"
        )
        # transmon.set_gate_shape("DragCosine")

        transmon.I.operations["saturation"] = pulses.SquarePulse(
            amplitude=0.25, length=32, digital_marker="ON"
        )

    if transmon.Q is not None:
        # Cosine pulse
        transmon.Q.operations["x180_Cosine"] = pulses.DragCosinePulse_Q(
            amplitude=0.1,
            alpha=0.0,
            anharmonicity=f"#/rf_qubits/{transmon.name}/anharmonicity",
            axis_angle=0,
        ...
        # DRAG pulses
        transmon.Q.operations["x180_DragCosine"] = pulses.DragCosinePulse_Q(
            amplitude=0.1,
            alpha=0.0,
            anharmonicity=f"#/rf_qubits/{transmon.name}/anharmonicity",
            axis_angle=0,
        ...
        transmon.Q.operations["x90_DragCosine"] = pulses.DragCosinePulse_Q(
            amplitude=0.1 / 2,
            alpha="#../x180_DragCosine/alpha",
            anharmonicity="#../x180_DragCosine/anharmonicity",
            axis_angle=0,
        ...
        # Square pulses - Q part is zero, I part in non-zero
        transmon.Q.operations["x180_Square"] = pulses.SquarePulse(
            amplitude=0.0, length=32, digital_marker="ON"
        )
        transmon.Q.operations["x90_Square"] = pulses.SquarePulse(
            amplitude=0.0 / 2, length="#../x180_Square/length", digital_marker="ON"
        )

        # transmon.set_gate_shape("DragCosine")

        transmon.Q.operations["saturation"] = pulses.SquarePulse(
            amplitude=0.0, length=20 * u.us, digital_marker="ON"
        )
