...
def instrument_limits(channel: Union[IQChannel, SingleChannel, MWChannel]) -> InstrumentLimits:
    if not (isinstance(channel, IQChannel) ^ isinstance(channel, MWChannel) ^ isinstance(channel, SingleChannel)):
        raise TypeError(
    ...
    elif isinstance(channel, IQChannel) or isinstance(channel, SingleChannel):
        limits = InstrumentLimits(
