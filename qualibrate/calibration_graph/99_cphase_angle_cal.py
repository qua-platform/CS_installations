...
with program() as stark_detuning:
    ...
    tomo = declare(bool) # to do tomography
    ...
    with for_(n, 0, n < n_avg, n + 1):
        ...
        with for_(*from_array(npi, N_pi_vec)):
            ...
            with for_(*from_array(df, dfs)):
                ...
                # Update coupler drive frequency
                update_frequency(c12.xy.name, df + c12.xy.intermediate_frequency)

                with for_each_(tomo, [True, False]):
                    ...
                    if reset_type == "active":
                        ...
                        wait(10_000)
                        qubits[0].resonator.measure("readout", qua_vars=(I[0], Q[0]))
                        qubits[1].resonator.measure("readout", qua_vars=(I[1], Q[1]))
                        assign(init_state[0], I[0] > qubit.resonator.operations["readout"].threshold)
                        assign(init_state[1], I[1] > qubit.resonator.operations["readout"].threshold)
                    ...
                    with if_(init_state[0] == False): # y90-tomo-pulse
                        qubit[0].xy_frame_rotation_2pi(0.5)
                        qubit[0].xy_play("x90_Cosine")
                        qubit[0].xy_reset_frame()

                    with elif_(init_state[0] == True): # -y90-tomo-pulse
                        qubit[0].xy_frame_rotation_2pi(-0.5)
                        qubit[0].xy_play("x90_Cosine")
                        qubit[0].xy_reset_frame()
                    ...
                    with if_(tomo == True): # -y90-tomo-pulse
                        qubit[0].xy_frame_rotation_2pi(-0.5)
                        qubit[0].xy_play('x90_Cosine')
                        qubit[0].xy_reset_frame()

                    with elif_(tomo == False): # x90-tomo-pulse
                        qubit[0].xy_play('x90_Cosine')
                    ...
                    if node.parameters.reset_type == "heralding":
                        ...
                        save(final_state[0], state_st[0])
                        assign(state[1], I[1] > qubit.resonator.operations["readout"].threshold)
                        assign(final_state[1], init_state[1] ^ state[1])
                        save(final_state[1], state_st[1])
    ...
    with stream_processing():
        ...
        for i, qubit in enumerate(qubits):
            if node.parameters.use_state_discrimination:
                state_st[i].boolean_to_int().buffer(2).buffer(len(dfs)).buffer(N_pi).average().save(f"state{i + 1}")
            else:
                I_stream = I_st[i].buffer(2).buffer(len(dfs)).buffer(N_pi).average().save(f"I{i + 1}")
                Q_stream = Q_st[i].buffer(2).buffer(len(dfs)).buffer(N_pi).average().save(f"Q{i + 1}")
...
if not node.parameters.simulate:
    ...
    if node.parameters.load_data_id is None:
        with node.record_state_updates():
            for qubit in qubits:
                qubit.I.operations[operation].detuning = float(fit_results[qubit.name]["detuning"])
