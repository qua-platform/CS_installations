    ...
    for i, qubit in enumerate(qubits):
        ...
        with for_(n, 0, n < n_avg, n + 1):
            ...
            with for_(*from_array(df, dfs)):
                ...
                qubit.align()
                duration = operation_len * u.ns if operation_len is not None else (qubit.I.operations[operation].length + qubit.z.settle_time) * u.ns
                ...
                qubit.xy_wait(qubit.z.settle_time * u.ns)
                qubit.xy_play(operation="x180_Cosine", amplitude_scale=operation_amp, duration=duration)
    ...
    for q in qubits:
        ...
        if not np.isnan(result.sel(qubit=q.name).position.values):
            ...
            print(
                ...
                f"{(result.sel(qubit = q.name).position.values + q.I.intermediate_frequency) / 1e9:.6f} GHz"
            )
            fit_results[q.name]["drive_freq"] = result.sel(qubit=q.name).position.values + q.I.intermediate_frequency
                ...
                if not np.isnan(result.sel(qubit=q.name).position.values):
                    if flux_point == "arbitrary":
                        q.arbitrary_intermediate_frequency = float(
                            result.sel(qubit=q.name).position.values + detunings[q.name] + q.I.intermediate_frequency
                        )
                    ...
                    if not flux_point == "arbitrary":
                        ...
                        if factor_cw * used_amp / operation_amp < limits.max_wf_amplitude:
                            q.I.operations["saturation"].amplitude = factor_cw * used_amp / operation_amp
                        else:
