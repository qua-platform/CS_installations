...
from quam_libs.lib.save_utils import fetch_results_as_xarray_CZ, load_dataset, get_node_id
...
with program() as power_rabi:
    ...
    with stream_processing():
        n_st.save("n")
        res_state.buffer(len(amps)).average().save("state1")
...
if not node.parameters.simulate:
    if node.parameters.load_data_id is None:
        ...
        qubits = [machine.active_qubits[0]]
        ds = fetch_results_as_xarray_CZ(job.result_handles, qubits, {"amp": amps})
    ...
    fit_results[c12.name] = {}
    f_fit = fit.loc[c12.name].sel(fit_vals="f")
    phi_fit = fit.loc[c12.name].sel(fit_vals="phi")
    ...
    fit_results[c12.name]["CZ_amplitude"] = new_pi_amp
    node.results["fit_results"] = fit_results


    # %% {Plotting}
    grid = QubitGrid(ds, q1.grid_location)
    for ax, qubit in grid_iter(grid):
        ds.assign_coords(amp_mV=ds.abs_amp * 1e3).loc[c12].state.plot(ax=ax, x="amp_mV")
        ax.plot(ds.abs_amp.loc[c12] * 1e3, fit_evals.loc[c12][0])
    ...
    if node.parameters.load_data_id is None:
        with node.record_state_updates():
            c12.operations['x180'].amplitude = fit_results[c12.name]["CZ_amplitude"]
            # for q in qubits:
            #     q.I.operations[operation].amplitude = fit_results[q.name]["CZ_amplitude"]
