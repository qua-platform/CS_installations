...
voltage_values_fast = np.linspace(-0.01, 0.01, n_points_fast)
    ...
    with for_(i, 0, i < n_points_slow + 1, i + 1):
        ...
        with for_(n, 0, n < n_avg, n + 1):  # The averaging loop
            with for_(*from_array(Vfast, voltage_values_fast)):
                ...
                set_dc_offset("Vd1", "single", Vfast)
...
if simulate:
    ...
    simulation_config = SimulationConfig(duration=25_000)  # In clock cycles = 4ns
    ...
    for i in range(n_points_slow):  # Loop over y-voltages
        ...
        # plt.cla()
        # plt.title("Phase [rad]")
        plt.pcolor(voltage_values_fast, voltage_values_slow[: iteration + 1], DC_signal)
        plt.xlabel("Fast voltage axis [V]")
        plt.ylabel("Slow voltage axis [V]")
        plt.tight_layout()
        plt.pause(0.1)
