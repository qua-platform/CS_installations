...
durations = np.arange(16, 20000, 1000)
pi_length = 100*u.ns
    ...
    with for_(n, 0, n < n_avg, n + 1):  # The averaging loop
        ...
        with for_(*from_array(t, durations)):  # Loop over the qubit pulse duration
            with strict_timing_():  # Ensure that the sequence will be played without gap
                ...
                seq.add_step(voltage_point_name="empty", duration = 5000)
                seq.add_step(voltage_point_name="load/wait", duration= t )
                seq.add_step(voltage_point_name="read", duration=readout_len)
                ...
                wait((duration_init) * u.ns + (t >> 2), "TIA")
