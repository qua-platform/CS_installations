    ...
    with program() as node.namespace["qua_program"]:
        ...
        with stream_processing():
            ...
            for i, qp in enumerate(qubit_pairs):
                state_c_st[i].buffer(2).buffer(3).buffer(len(pulse_durations)).buffer(len(phases)).average().save(f"state_c{i}")
                state_t_st[i].buffer(2).buffer(3).buffer(len(pulse_durations)).buffer(len(phases)).average().save(f"state_t{i}")
