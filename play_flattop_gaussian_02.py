...
relative_phases = 2 * np.pi * np.arange(-1.0, 1.0, 0.1)
...
with program() as PROGRAM:
    ...
    cos = declare(fixed, value=cos_py)
    sin = declare(fixed, value=sin_py)
    ...
    with for_(n, 0, n < n_avg, n + 1):  # QUA for_ loop for averaging
        with for_(*from_array(t, flattop_durations)):  # QUA for_ loop for sweeping the frequency
            with for_(i, 0, i < len(relative_phases), i + 1):
                play("rise" * amp(cos[i], -sin[i], sin[i], cos[i]), qb1)
                wait(4, qb2)

                play("flattop" * amp(cos[i], -sin[i], sin[i], cos[i]), qb2, duration=t)
                wait(t, qb1)

                play("fall" * amp(cos[i], -sin[i], sin[i], cos[i]), qb1)
