...
t_vec = np.arange(x90_len_NV//4 + 4, 250, 10)  # The time vector for varying the idle time in clock cycles (4ns)
...
with program() as ramsey:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(*from_array(t, t_vec)):
            ...
            wait(t - x90_len_NV//4 , "NV")  # Variable idle time
            play("x90" * amp(1), "NV")  # Pi/2 pulse to qubit
            ...
            if wait_pulse_time_for_background_measuremenets:
                ...
                wait(t - x90_len_NV//4, "NV")  # variable delay in spin Echo
                play("x90" * amp(0), "NV")  # Pi/2 pulse to qubit
            align()  # Play the laser pulse after the Ramsey sequence
...
if simulate:
    ...
    while results.is_processing():
        ...
        # Plot data
        plt.cla()
        plt.plot(4 * t_vec, counts / 1000 / (meas_len / u.s), label="x90_idle_x90")
