...
t_vec = np.arange(x180_len_NV//4 + 4, 500, 20)  # The time vector for varying the idle time in clock cycles (4ns)
n_avg = 1_000_000
...
with program() as hahn_echo:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(*from_array(t, t_vec)):
            ...
            wait(t - x180_len_NV//4, "NV")  # Variable idle time
            ...
            if wait_pulse_time_for_background_measuremenets:
                # Second Ramsey sequence for measuring the dark counts
                play("x90" * amp(0), "NV")  # Pi/2 pulse to qubit
                wait(t - x180_len_NV//4, "NV")  # Variable idle time
                play("x180" * amp(0), "NV")  # Pi pulse to qubit
                wait(t - x180_len_NV//4, "NV")  # variable delay in spin Echo
                play("x90" * amp(0), "NV")  # Pi/2 pulse to qubit
...
if simulate:
    ...
    while results.is_processing():
        ...
        # Plot data
        plt.cla()
        plt.plot(8 * t_vec, counts / 1000 / (meas_len / u.s), label="x90_idle_x180_idle_x90")
