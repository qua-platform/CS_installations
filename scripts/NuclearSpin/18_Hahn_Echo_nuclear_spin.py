...
and measure the photon counts received by the APD across varying idle times.
...
# The QUA program #
###################
with program() as hahn_echo_nuclear_spin:
...
if simulate:
    ...
    job = qmm.simulate(config, hahn_echo_nuclear_spin, simulation_config)
    ...
    # execute QUA program
    job = qm.execute(hahn_echo_nuclear_spin)
    # Get results from QUA program
    results = fetching_tool(job, data_list=["counts1", "counts_dark", "iteration"], mode="live")
    # Live plotting
    fig = plt.figure()
    ...
    while results.is_processing():
        # Fetch results
        counts1, counts_dark, iteration = results.fetch_all()
        # Progress bar
        progress_counter(iteration, n_avg, start_time=results.get_start_time())
        # Plot data
        plt.cla()
        plt.plot(8 * t_vec, counts1 / 1000 / (meas_len / u.s), label="x90_idle_x180_idle_x90")
        plt.plot(8 * t_vec, counts_dark / 1000 / (meas_len / u.s), label="dark counts")
