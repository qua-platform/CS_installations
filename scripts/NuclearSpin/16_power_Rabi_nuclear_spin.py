"""
        POWER RABI
The program consists in playing a mw pulse and measure the photon counts received by the APD
...
# The QUA program #
###################
with program() as power_rabi_nuclear_spin:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(*from_array(a, a_vec)):
            ...
            play("x180" * amp(a), "RF")  # 'a' is a pre-factor to the amplitude defined in the config ("mw_amp_NV")
...
if simulate:
    ...
    job = qmm.simulate(config, power_rabi_nuclear_spin, simulation_config)
    ...
    job = qm.execute(power_rabi_nuclear_spin)
