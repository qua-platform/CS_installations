...
with program() as prog:
    ...
    with for_(i, 0, i < 5, i + 1):
        with for_(n, 0, n < 10, n + 1):
            play("gauss", "photon_source", condition= Random().rand_fixed() > 0.7)  # plays single_photon operation on qubit
            wait(4, "photon_source")  # wait 4 clock cycles (16 ns)

    measure("readout", "SPCM1", adc_st, time_tagging.analog(times, meas_len, counts))  # measures on SPCM
