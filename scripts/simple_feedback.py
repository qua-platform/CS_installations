...
with program() as simple_feedback:
    ...
    with for_(n, 0, n < n_tries, n + 1):
        ...
        measure(
            "readout", "tank_circuit", None, demod.full("cos", I, "out2")
        )  # , demod.full("sin", Q, "out2"))
        save(I, I_st)
        with if_(I > threshold):  # On peak
            seq.add_step(voltage_point_name="off_peak")
            save(1, on_off_st)
        with elif_(I < threshold):  # Off peak
...
if simulate:
    ...
    results = fetching_tool(
        job, data_list=["on_off", "I", "iteration"], mode="wait_for_all"
    )
