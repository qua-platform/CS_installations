        ...
        if self.result_handles is None:
            ...
            if "I" in output.keys() and "Q" in output.keys():
                ...
                if self.unwrap_phase is True:
                    output["Phi"] = (
                        np.unwrap(np.angle(output["I"] + 1j * output["Q"]))
                        * 180
                        / np.pi
                    )
                else:
                    output["Phi"] = (
                        np.angle(output["I"] + 1j * output["Q"]) * 180 / np.pi
                    )
            return output
        ...
        if len(gene.values) > 0:
            if gene.values[0].string_value == "saveAll":
                ...
                if not averaging_buffer:
                    ...
                    if hasattr(gene.values[2].list_value.values[1], "list_value"):
                        if (
                            len(gene.values[2].list_value.values[1].list_value.values)
                            > 0
                        ):
                            if (
                                gene.values[2]
            ...
            if len(gene.values) > 2:
                if hasattr(gene.values[2], "list_value"):
                    self._extend_result(
                        gene.values[2].list_value, count, averaging_buffer
                    )
            elif gene.values[0].string_value == "average":
                if hasattr(gene.values[1], "list_value"):
                    self._extend_result(
                        gene.values[1].list_value, count, averaging_buffer
                    )
