...
n_avg = 3
num_of_sequences = 4  # Number of random sequences
circuit_depth_min = 0
circuit_depth_max = 5 # worked up to 7800
delta_clifford = 1
...
with program() as rb:
    ...
    with stream_processing():
        ...
        ss_st.buffer(circuit_depth_max).buffer(num_of_sequences).buffer(len(circuit_depths)).save("rb_sequences")
        P_diff_st.buffer(n_avg).map(FUNCTIONS.average())\
            .buffer(num_of_sequences).map(FUNCTIONS.average())\
            .buffer(len(circuit_depths)).save(f"P_diff_avg_{tank_circuit}")
...
if simulate:
    ...
    iteration, P_diff, depths, rb_sequences = results.fetch_all()
