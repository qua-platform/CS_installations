...
with program() as raw_trace_prog:
    ...
    with stream_processing():
        ...
        # Save the raw ADC traces as real and image parts
...
if simulate:
    ...
    plt.plot(adc1_single_run.real, label="Input 1 (I)")
    plt.plot(adc1_single_run.imag, label="Input 1 (Q)")
    plt.xlabel("Time [ns]")
    ...
    plt.plot(adc1.real, label="Input 1 (I)")
    plt.plot(adc1.imag, label="Input 1 (Q)")
    plt.xlabel("Time [ns]")
    # plt.legend()

    # fft of adc1
    plt.subplot(133)
    plt.title("FFT of averaged run")
    adc1_I_freq = np.fft.fft(adc1.real)
    adc1_Q_freq = np.fft.fft(adc1.imag)
    adc1_freq = np.fft.fftfreq(len(adc1))
    plt.plot(adc1_freq, np.abs(adc1_I_freq), label="Input 1 (I)")
    plt.plot(adc1_freq, np.abs(adc1_Q_freq), label="Input 1 (Q)")
    plt.axvline(resonator_IF / u.GHz, color="k", linestyle="--", label="IF")
    plt.xlabel("Frequency [GHz]")
    plt.xlim(xmin=0, xmax=max(adc1_freq))
    plt.legend()
