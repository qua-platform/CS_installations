...
def fit_decay_exp(da, dim):
    def get_decay(dat):
        def oed(d): return ca.guess.exp_decay(da[dim], d)
    ...
    decay_guess = xr.apply_ufunc(get_decay, da, input_core_dims=[
                                 [dim]]).rename('decay guess')
    amp_guess = xr.apply_ufunc(get_amp, da, input_core_dims=[
                               [dim]]).rename('amp guess')
    min_guess = xr.apply_ufunc(get_min, da, input_core_dims=[
                               [dim]]).rename('min guess')
    ...
    fit_res = xr.apply_ufunc(apply_fit, da[dim], da, amp_guess, min_guess, decay_guess,
...
def fit_oscillation_decay_exp(da, dim):
    def get_decay(dat):
        def oed(d): return ca.guess.oscillation_exp_decay(da[dim], d)
        return np.apply_along_axis(oed, -1, dat)
    ...
    decay_guess = xr.apply_ufunc(get_decay, da, input_core_dims=[
                                 [dim]]).rename('decay guess')
    freq_guess = xr.apply_ufunc(get_freq, da, input_core_dims=[
                                [dim]]).rename('freq guess')
    amp_guess = xr.apply_ufunc(get_amp, da, input_core_dims=[
                               [dim]]).rename('amp guess')
    ...
    fit_res = xr.apply_ufunc(apply_fit, da[dim], da, amp_guess, freq_guess, 0, 0.5,
                             decay_guess,
                             input_core_dims=[
                                 [dim], [dim], [], [], [], [], []],
                             output_core_dims=[['fit_vals']],
...
def fit_echo_decay_exp(da, dim):
    ...
    def apply_fit(x, y, a, offset, decay, decay_echo):
        try:
            fit = curve_fit(echo_decay_exp, x, y, p0=[
                            a, offset, decay, decay_echo])[0]
            return fit
        except RuntimeError as e:
    ...
    fit_res = xr.apply_ufunc(apply_fit, da[dim], da, amp_guess, -0.0005, decay_guess,
                             decay_guess,
                             input_core_dims=[[dim], [dim], [], [], [], []],
                             output_core_dims=[['fit_vals']],
                             vectorize=True,
...
def fit_oscillation(da, dim):
    ...
    freq_guess = fix_initial_value(xr.apply_ufunc(
        get_freq, da_c, input_core_dims=[[dim]]).rename('freq guess'), da_c)
    amp_guess = fix_initial_value(xr.apply_ufunc(
        get_amp, da, input_core_dims=[[dim]]).rename('amp guess'), da)
    ...
    def apply_fit(x, y, a, f, phi, offset):
        try:
            ...
            fit = model.fit(y, t=x, a=Parameter("a", value=a, min=0), f=Parameter(
                "f", value=f, min=np.abs(0.5*f), max=np.abs(f*3+1e-3)), phi=Parameter('phi', value = phi) , offset=offset)
            if fit.rsquared < 0.9:    
                fit = model.fit(y, t=x, a=Parameter("a", value=a, min=0), f=Parameter(
               "f", value=1.0 / (np.max(x) - np.min(x)), min=0, max=np.abs(f*3+1e-3)), phi=Parameter('phi', value = phi), offset=offset)  
    ...
    fit_res = xr.apply_ufunc(apply_fit, da[dim], da, amp_guess, freq_guess, phase_guess, offset_guess,
                             input_core_dims=[[dim], [dim], [], [], [], []],
                             output_core_dims=[['fit_vals']],
                             vectorize=True
                             )
