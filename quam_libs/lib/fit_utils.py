...
def _S21_abs(w, A, k, phi, kappa_p, omega_p, omega_r, J):
    ...
    return (A + k*w) * np.abs(
        np.cos(phi) - np.exp(1j*phi) * kappa_p * (- 2 * 1j * Delta_r) / (
            4 * J**2 + (kappa_p - 2 * 1j * Delta_p) *
            (-2 * 1j * Delta_r))
    )
...
def _truncate_data(transmission, window):
    ...
    truncated_transmission = transmission.sel(freq=slice(peak_freq-window,
                                                         peak_freq+window))
...
def _guess_2_resonators(transmission):
    ...
    k, A = (-first.base_line).polyfit(dim='freq',
                                      deg=1).polyfit_coefficients.values
...
def fit_resonator_purcell(s21_data: xr.Dataset,
                          init_J: float = 15e6,
...
def _guess_single(transmission, frequency_LO_IF, rolling_window, window):
    ...
    def find_upper_envelope(transmission, rolling_window):
        rolling_transmission = transmission.IQ_abs.rolling(
            freq=rolling_window, center=True).mean()
    ...
    Q = frequency_LO_IF / \
        np.abs((transmission.IQ_abs.diff(
            dim='freq').idxmin(dim='freq')-omega_r)).values
    Q = Q if Q < 1e4 else 1e4
    Q = 1e4
    Qe = Q/(1 - transmission.IQ_abs.min(dim='freq') /
            transmission.IQ_abs.max(dim='freq'))
...
class _single_resonator(Model):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # params used in the initial guess generator:
        # used to smooth dtata to imrove peak detection
    ...
    def make_fit(self, transmission, frequency_LO_IF, init_guess=None):
        ...
        if init_guess == None:
            init_guess = _guess_single(transmission_trunc, frequency_LO_IF=frequency_LO_IF,
                                       rolling_window=self.rolling_window,
                                       window=self.window)

        data = (transmission_trunc.IQ_abs *
                np.exp(1j * transmission_trunc.phase)).values
        f = transmission_trunc.freq.values
...
def fit_resonator(s21_data: xr.Dataset,
