...
with program() as ramsey_freq_duration:
    ...
    with for_(n, 0, n < n_avg, n + 1):  # QUA for_ loop for averaging
        with for_(*from_array(flux, fluxes)):  # QUA for_ loop for sweeping the idle time
            with for_(*from_array(tau, taus)):  # QUA for_ loop for sweeping the qubit frequency
    ...
    with stream_processing():
        ...
        I_st.buffer(len(taus)).buffer(len(fluxes)).average().save("I")
        Q_st.buffer(len(taus)).buffer(len(fluxes)).average().save("Q")
        n_st.save("iteration")
