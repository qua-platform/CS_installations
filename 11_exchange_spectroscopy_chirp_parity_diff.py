...
freqs = np.arange(140e6, 190e6, 0.2e6)

seq_B = VoltageGateSequence(config, [barrier_gate])
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(*from_array(Vb, voltages_B)):
            with for_(*from_array(f, freqs)):
                ...
                align()
                seq.add_step(
                    voltage_point_name="initialization_1q",
                    duration=RF_SWITCH_DELAY + delay_init_qubit_start + delay_barrier_gate + CONST_LEN + delay_init_qubit_end + RF_SWITCH_DELAY,
                    ramp_duration=duration_ramp_init,
                ) # NEVER u.ns
                
                # seq_B.add_step(voltage_point_name="wait", duration=delay_barrier_gate + RF_SWITCH_DELAY)

                align("rf_switch", qubit)
                wait((delay_barrier_gate + RF_SWITCH_DELAY) * u.ns, barrier_gate)
                seq_B.add_step(level=[Vb,], duration=RF_SWITCH_DELAY + delay_barrier_crot + pi_len + delay_barrier_crot, ramp_duration=duration_ramp_barrier)
                ...
                wait(delay_barrier_crot // 4, qubit)
                # play("x180_kaiser", qubit)

                align(qubit, barrier_gate)
                seq_B.add_step(voltage_point_name="readout", duration=duration_ramp_barrier, ramp_duration=duration_ramp_barrier)
...
if simulate:
    ...
    while results.is_processing():
        ...
        plt.pcolor(freqs / u.MHz, voltages_B, P_diff_avg)
        plt.xlabel("Freq [MHz]")
