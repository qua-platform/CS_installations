...
level_readout_min = [-0.05, 0.05]
level_readout_max = [0.1, -0.1]
...
with program() as PSB_search_prog:
    ...
    with stream_processing():
        n_st.save("iteration")
        for j, tc in enumerate(tank_circuits):
            I_st[j].buffer(n_detunings).save_all(f"I_{tc}")
            Q_st[j].buffer(n_detunings).save_all(f"Q_{tc}")
...
if simulate:
    ...
    data_handler.save_data(data=save_data_dict, name="07_PSB_search_opx")
