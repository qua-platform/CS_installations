...
import numpy as np
from qualang_tools.config.waveform_tools import drag_gaussian_pulse_waveforms, drag_cosine_pulse_waveforms, flattop_gaussian_waveform
...
qubit_full_scale_power_dbm_q1 = -11 #-11 #Lo
qubit_full_scale_power_dbm_q2 = -11 #Lo
qubit_full_scale_power_dbm_q3 = -11 #Lo
qubit_full_scale_power_dbm_q4 = 1 #Lo
...
qubit_band_q6 = 1
# Qubits LO
qubit_LO_q1 = 4.80 * u.GHz # = 5.1e9
qubit_LO_q2 = 5.00 * u.GHz
qubit_LO_q3 = 4.50 * u.GHz
qubit_LO_q4 = 0.5 * u.GHz
...
resonator_band = 2
# Resonators LO
resonator_LO = 5.60 * u.GHz
# Resonators IF
resonator_IF_q1 = int(-247 * u.MHz)
resonator_IF_q2 = int(-110 * u.MHz)
resonator_IF_q3 = int(70 * u.MHz)
resonator_IF_q4 = int(195.3 * u.MHz)
resonator_IF_q5 = int(318 * u.MHz)
resonator_IF_q6 = int(449* u.MHz)
# resontor_delay
resonator_delay = 0
...
readout_amp_q2 = 0.03
readout_amp_q3 = 0.03
readout_amp_q4 = 0.05# 0.03
readout_amp_q5 = 0.03# 0.05
readout_amp_q6 = 0.03

# TOF and depletion time
time_of_flight = 28 + 368  #unit is ns # must be a multiple of 4 # Can not lower than 28 
...
rotation_angle_q4 = (270 / 180) * np.pi #(292.5 / 180) * np.pi
...
readout_gf =np.array(
    flattop_gaussian_waveform(readout_amp_q4, readout_len-60, rise_fall_length = 30, return_part="all", sampling_rate=1e9)
)
...
config = {
    ...
    "waveforms": {
        ...
        "readout_wf_q4": {"type": "arbitrary", "samples": readout_gf.tolist()},
        #------------------
    ...
    "integration_weights": {
        ...
        "rotated_cosine_weights_q4": {
            "cosine": [(np.cos(rotation_angle_q4), demod_len)],
            "sine": [(np.sin(rotation_angle_q4), demod_len)],
        },
        "rotated_sine_weights_q4": {
            "cosine": [(-np.sin(rotation_angle_q4), demod_len)],
            "sine": [(np.cos(rotation_angle_q4), demod_len)],
        },
