...
if simulate:
    ...
    try:
        ...
        # Cosine fit on minima vs current
        def cosine_func(x, amplitude, frequency, phi, offset):
            return amplitude * np.cos(2 * np.pi * frequency * x + phi) + offset

        minima = np.zeros(M)
        for j in range(M):
            minima[j] = frequencies[resonator][np.argmin(R[:, j])]

        initial_guess = [1, 1 / 0.4, 0, 0]
        fit_params, _ = curve_fit(cosine_func, currents, minima, p0=initial_guess)
        amplitude_fit, frequency_fit, phase_fit, offset_fit = fit_params
        fitted = cosine_func(currents, amplitude_fit, frequency_fit, phase_fit, offset_fit)
        print("fitting parameters", fit_params)
        print("Current corresponding to the maximum frequency point (A)", currents[np.argmax(fitted)])

        plt.figure()
        plt.suptitle(f"Resonator spectroscopy - LO = {resonator_LO / u.GHz} GHz")
        plt.pcolor(currents, (frequencies[resonator]) / u.MHz, R)
        plt.plot(currents, minima / u.MHz, "x-", color="red", label="Minima")
        plt.plot(currents, fitted / u.MHz, color="orange", label="Fitted Cosine")
        plt.xlabel("Current [A]")
