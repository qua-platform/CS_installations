...
with program() as calib_mw_delays:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        ...
        wait(initial_delay_cycles, "OpticalTrigger", "EOM", "AOM1", "AOM2", "RF_switch")  # wait before starting PL, replace with elements not testing currently
        #After calibrating drive 1 (mwe), switch out "RF_switch" and mwe
        EOM.play("const_pulse", duration=laser_len_cycles)
        AOM1.play("AOM1_ON", duration=laser_len_cycles)
        AOM2.play("AOM2_ON", duration = laser_len_cycles)
        OpticalTrigger.play("Laser_ON", duration=laser_len_cycles)  # Cycle through each of these for various delays
        RF_switch.play("RF_ON", duration=laser_len_cycles)

        wait(initial_delay_cycles + (laser_len_cycles - var_len_cycles) // 2, "mwe")  # delay the pulse until
        mwe.play("const_pulse", duration = var_len_cycles)  # play microwave pulse
