# %%
"""
        echo-Cross-Resonance (flattop pulse) Power Rabi with single-qubit Quantum State Tomography
...
def play_flattop(cr: str, amplitude: int, sign: str):
    ...
    if sign == "positive":
        ...
        wait(int(cr_c1t2_rise_fall_len // 4), cr)
        play("flattop" * amp(-1 * amplitude), cr)
        wait(int(cr_c1t2_rise_fall_len // 4), cr)
        ...
        with for_(*from_array(a, a_vec)):
            with for_(c, 0, c < 3, c + 1): # bases
                with for_(s, 0, s < 2, s + 1): # states
                    ...
                    # control - CR
                    play_flattop("cr_c1t2", a, sign="positive")
                    align()
                    play("x180", "q1_xy")
                    align()
                    play_flattop("cr_c1t2", a, sign="negative")
                    align()
                    play("x180", "q1_xy")
                    align()
