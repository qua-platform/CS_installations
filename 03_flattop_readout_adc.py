...
with program() as prog:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        ...
        measure(
            "gaussian_rise_readout" * amp(a), f"{rr}_twin",
            adc_rise,
            dual_demod.full("cos", "minus_sin", I_rise),
            dual_demod.full("sin", "cos", Q_rise),
        )
        wait(READOUT_GAUSSIAN_RISE_LEN * u.ns, rr)
        measure(
            "flattop_readout" * amp(a), rr,
            adc_flat,
            dual_demod.full("cos", "minus_sin", I_flat),
            dual_demod.full("sin", "cos", Q_flat),
        )
        wait(READOUT_FLATTOP_LEN * u.ns, f"{rr}_twin")
        measure(
            "gaussian_fall_readout" * amp(a), f"{rr}_twin",
            adc_fall,
