...
Before proceeding to the next node:
    - Adjust the qubit frequency setting, labeled as "qubit_IF_q1", in the configuration.
    - Modify the qubit pulse amplitude setting, labeled as "pi_amp_q1", in the configuration.
"""
    ...
    with for_(n, 0, n < n_avg, n + 1):  # QUA for_ loop for averaging
        with for_(*from_array(a, amplitudes)):  # QUA for_ loop for sweeping the pulse amplitude pre-factor
            with for_(*from_array(f, dfs)):  # QUA for_ loop for sweeping the frequency
                ...
                update_frequency("q1_xy", f + qubit_IF_q1)
...
if simulate:
    ...
    while results.is_processing():
        ...
        # Plot results
        plt.subplot(211)
        plt.suptitle(f"Rabi chevron with LO={qubit_LO_q1 / u.GHz}GHz and IF={qubit_IF_q1 / u.MHz}MHz")
        plt.cla()
        plt.title(r"$R=\sqrt{I^2 + Q^2}$")
        plt.pcolor(dfs / u.MHz, amplitudes * pi_amp_q1, R)
        plt.xlabel("Frequency detuning [MHz]")
        plt.ylabel("Pulse amplitude [V]")
        plt.subplot(212)
        plt.cla()
        plt.title("Phase")
        plt.pcolor(dfs / u.MHz, amplitudes * pi_amp_q1, np.unwrap(phase))
        plt.xlabel("Frequency detuning [MHz]")
        plt.ylabel("Pulse amplitude [V]")
        plt.tight_layout()
        plt.pause(0.1)
