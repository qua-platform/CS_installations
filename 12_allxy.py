...
if simulate:
    ...
    data_list = ["iteration"] + list(
        np.concatenate([[f"I{i}", f"Q{i}"] for i in range(len(sequence))])
    )
    ...
    while results.is_processing():
        ...
        plt.plot(
            [np.max(I)] * 5 + [np.mean(I)] * 12 + [np.min(I)] * 4,
            "r-",
            label="Expected value",
        )
        plt.ylabel("I quadrature [a.u.]")
        plt.xticks(
            ticks=range(len(sequence)), labels=["" for _ in sequence], rotation=45
        )
        plt.legend()
        plt.subplot(212)
        plt.cla()
        ...
        plt.plot(
            [np.max(Q)] * 5 + [np.mean(Q)] * 12 + [np.min(Q)] * 4,
            "r-",
            label="Expected value",
        )
        plt.ylabel("Q quadrature [a.u.]")
        plt.xticks(
            ticks=range(len(sequence)), labels=[str(el) for el in sequence], rotation=45
        )
        plt.legend()
        plt.tight_layout()
        plt.pause(0.1)
