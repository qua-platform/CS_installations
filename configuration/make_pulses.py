...
def add_default_transmon_pulses(transmon):
    ...
    transmon.xy.operations["-x90_Square"] = pulses.SquarePulse(
        amplitude=-0.25 / 2,
        length="#../x180_Square/length",
        axis_angle=0, digital_marker="ON"
    )
...
def add_default_transmon_pair_pulses(transmon_pair):
    transmon_pair.cr_control.operations["square"] = pulses.SquarePulse(
        amplitude=0.1,
        length=100,
    )
    transmon_pair.cr_target.operations["square"] = pulses.SquarePulse(
        amplitude=0.1,
        length=100,
    )
