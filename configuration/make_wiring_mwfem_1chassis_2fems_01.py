...
q_res_chs = [
    [[ 1, 2, 3, 4, 5, 6, 7, 8], mw_fem_spec(con=1, slot=1, in_port=1, out_port=1)], # FEM1 - RL1
]

qb_chs = [
    [[1], mw_fem_spec(con=1, slot=2, out_port=1)],
    [[2], mw_fem_spec(con=1, slot=2, out_port=2)],
    [[3], mw_fem_spec(con=1, slot=2, out_port=3)],
    [[4], mw_fem_spec(con=1, slot=2, out_port=4)],
    [[5], mw_fem_spec(con=1, slot=2, out_port=5)],
    [[6], mw_fem_spec(con=1, slot=2, out_port=6)],
    [[7], mw_fem_spec(con=1, slot=2, out_port=7)],
    [[8], mw_fem_spec(con=1, slot=2, out_port=8)],
]
...
for qp in qubit_pairs:
    ...
    allocate_wiring(connectivity, instruments, block_used_channels=False)
    # zz-induced interaction
    connectivity.add_qubit_pair_zz_drive_lines(qubit_pairs=[qp], constraints=qc_constraint)
    allocate_wiring(connectivity, instruments, block_used_channels=False)
