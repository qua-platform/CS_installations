...
for i, qubit in enumerate(qubits):
    ...
    connectivity.add_qubit_flux_lines(qubits=qubit, constraints=q_flux_chs[i])
    if i < len(qubits)-1:
        connectivity.add_qubit_pair_flux_lines(qubit_pairs=[(qubits[i], qubits[i + 1])], constraints=coupler_res_ch[i])
