...
with program() as prog:
    ...
    with stream_processing():
        ...
        for ind, rr in enumerate(resonators):
            I_st[ind].buffer(len(ts_cycle)).buffer(len(dfs)).average().save(f"I_{rr}")
            Q_st[ind].buffer(len(ts_cycle)).buffer(len(dfs)).average().save(f"Q_{rr}")
...
if __name__ == "__main__":
    ...
    if simulate:
        ...
        try:
            ...
            fig, axss = plt.subplots(2, 2, figsize=(8, 5), sharex=True, sharey=True)
            ...
            while results.is_processing():
                ...
                for ax, Vname, V in zip(axss.ravel(), fetch_names[1:], res[1:]):
                    ax.pcolor(ts_ns, dfs, V)
                    ax.set_xlabel("Idle time [ns]")
                    ax.set_ylabel("Freq detuning [Hz]")
                    ax.set_title(Vname)
                plt.tight_layout()
                plt.pause(0.1)
            ...
            St = It + 1j * Qt
            detuning_qt = np.zeros(len(dfs))
            for i, df in enumerate(dfs):
                try:
                    fig_analysis = plt.figure()
                    fit = Fit()
                    ramsey_fit = fit.ramsey(ts_ns, np.abs(St[i, :]), plot=True)
                    qb_T2 = np.abs(ramsey_fit["T2"][0])
                    detuning_qt[i] = ramsey_fit["f"][0] * u.GHz - freq_detuning
                    plt.xlabel("Idle time [ns]")
                    plt.ylabel("abs(I + iQ) [V]")
                    plt.legend((f"qubit detuning = {-detuning_qt[i] / u.kHz:.3f} kHz", f"T2* = {qb_T2:.0f} ns"))
                    plt.title(f"Ramsey with off-resonant drive at df = {df}")
                except (Exception,):
                    pass
                finally:
                    save_data_dict.update({f"fig_analysis_target_{i:03d}_df={df/1e6:5.2f}MHz": fig_analysis})
            
            fig_summary = plt.figure()
            plt.plot(dfs, detuning_qt)
            plt.xlabel("Freq detuning [Hz]")
            plt.xlabel("Qubit detuning [Hz]")
            plt.title("Off-resonant Stark shift")
            ...
            data_handler.save_data(data=save_data_dict, name="off_resonant_Stark_shift_vs_amplitude")
