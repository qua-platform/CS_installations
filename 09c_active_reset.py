...
Prerequisites:
    ...
    - Having calibrated the IQ blobs (rotation_angle and ge_threshold_q1).
    - (optional) Having calibrated the readout (readout_frequency_, _amplitude_, _duration_optimization).
    - Having updated the rotation angle (rotation_angle) and g -> e threshold (ge_threshold_q1) in the configuration (IQ_blobs.py).
"""
...
ge_threshold_q1_g = ge_threshold_q1 * 0.5
ge_threshold_q1_e = ge_threshold_q1
...
def qubit_initialization(method: str = "thermalization"):
    ...
    if method == "thermalization":
        ...
        return active_reset_fast(ge_threshold_q1_e)
    elif method == "active_reset_one_threshold":
        return active_reset_one_threshold(ge_threshold_q1_e, max_tries)
    elif method == "active_reset_two_thresholds":
        return active_reset_two_thresholds(ge_threshold_q1_g, ge_threshold_q1_e, max_tries)
    else:
...
def active_reset_one_threshold(threshold_g: float, max_tries: int):
    ...
    with while_((I_reset > threshold_g) & (counter < max_tries)):
        ...
        measure("readout", "rr1", None, dual_demod.full("rotated_cos", "out1", "rotated_sin", "out2", I_reset))
        align("rr1", "q1_xy")
        # Wait for the resonator to deplete
        wait(depletion_time * u.ns, "q1_xy")
        # Play a conditional pi-pulse to actively reset the qubit
        play("x180", "q1_xy", condition=(I_reset > threshold_g))
...
def active_reset_two_thresholds(threshold_g: float, threshold_e: float, max_tries: int):
    ...
    with while_((I_reset > threshold_g) & (counter < max_tries)):
        ...
        wait(depletion_time * u.ns, "q1_xy")
        # Play a conditional pi-pulse to actively reset the qubit
        play("x180", "q1_xy", condition=(I_reset > threshold_e))
        # Update the counter for benchmarking purposes
        assign(counter, counter + 1)
    return counter
