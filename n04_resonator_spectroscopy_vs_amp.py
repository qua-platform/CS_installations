...
if simulate:
    ...
    while results.is_processing():
        ...
        plt.pcolor(dfs / u.MHz, amplitudes * readout_amp, R.T)
        plt.yscale('log')
        plt.ylim(amplitudes[0] * readout_amp, amplitudes[-1] * readout_amp)
        plt.xlabel("Readout detuning [MHz]")
        plt.subplot(212)
        plt.cla()
        plt.title("Phase")
        # plt.pcolor(amplitudes * readout_amp, dfs / u.MHz, signal.detrend(np.unwrap(phase)))
        plt.pcolor(dfs / u.MHz, amplitudes * readout_amp, phase.T)
        plt.yscale('log')
        plt.ylim(amplitudes[0] * readout_amp, amplitudes[-1] * readout_amp)
        plt.xlabel("Readout detuning [MHz]")
