...
Next steps before going to the next node:
    - Update the qubit pulse amplitude (pi_amp_q1) in the configuration.
"""
...
if simulate:
    ...
    while results.is_processing():
        ...
        plt.subplot(221)
        plt.cla()
        plt.pcolor(amplitudes * pi_amp_q1, nb_of_pulses, I)
        plt.xlabel("Rabi pulse amplitude [V]")
        plt.ylabel("# of Rabi pulses")
        plt.title("I quadrature [V]")
        plt.subplot(222)
        plt.cla()
        plt.pcolor(amplitudes * pi_amp_q1, nb_of_pulses, Q)
        plt.xlabel("Rabi pulse amplitude [V]")
        plt.title("Q quadrature [V]")
        plt.subplot(212)
        plt.cla()
        plt.plot(amplitudes * pi_amp_q1, np.sum(I, axis=0))
        plt.xlabel("Rabi pulse amplitude [V]")
        plt.ylabel("Sum along the # of Rabi pulses")
        plt.pause(0.1)
        plt.tight_layout()
    print(f"Optimal pi_amp_q1 = {amplitudes[np.argmin(np.sum(I, axis=0))] * pi_amp_q1:.4f} V")
