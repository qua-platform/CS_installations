...
# gauss rise
gauss_rise_short_wf = generate_gaussian_waveform(
    amplitude=GAUSS_AMP,
...
# gauss fall
gauss_fall_short_wf = generate_gaussian_waveform(
    amplitude=GAUSS_AMP,
    length=GAUSS_RISE_FALL_LEN,
    sigma_ratio=0.2,
...
# gauss rise
gauss_rise_long_wf = generate_gaussian_waveform(
    amplitude=GAUSS_AMP,
    length=2 * GAUSS_RISE_FALL_LEN,
    sigma_ratio=0.2,
    rise_fall="rise", # rise or fall
)
# gauss fall
gauss_fall_long_wf = generate_gaussian_waveform(
    amplitude=GAUSS_AMP,
    length=2 * GAUSS_RISE_FALL_LEN,
    sigma_ratio=0.2,
    rise_fall="fall", # rise or fall
...
for d in range(1, 21, 1):
    flat_ns = 4 * d 
    gauss_flattop_wfs[flat_ns] = np.array(
        gauss_rise_short_wf.tolist() +\
        [GAUSS_AMP] * flat_ns +\
        gauss_fall_short_wf.tolist()
    )
...
config = {
        ...
        **{
            f"qubit_{con}-fem{fem}-port{p}-duc{duc}{tw}": {
                ...
                "operations": {
                    ...
                    **{
                        f"gauss_flattop_{flat_ns:03d}": f"gauss_flattop_{flat_ns}_pulse"
                        for flat_ns, wf in gauss_flattop_wfs.items()
                    },
                    "arb": "arb_pulse",
                },
                "core": f"{con}-fem{fem}-port{p}-duc{duc}{tw}",
            }
            for tw in ["", "-twin"]
            for duc in [1, 2]
            for p in MW_OUTPUT_PORTS
    ...
    "pulses": {
        ...
        **{
            f"gauss_flattop_{flat_ns}_pulse": {
                "operation": "control",
                "length": len(wf),
                "waveforms": {
                    "I": f"gauss_flattop_{flat_ns}_wf",
                    "Q": "zero_wf",
                },
            }
            for flat_ns, wf in gauss_flattop_wfs.items()
    ...
    "waveforms": {
        ...
        "gauss_rise_short_wf": {"type": "arbitrary", "samples": gauss_rise_short_wf.tolist()},
        "gauss_fall_short_wf": {"type": "arbitrary", "samples": gauss_fall_short_wf.tolist()},
        "gauss_rise_long_wf": {"type": "arbitrary", "samples": gauss_rise_long_wf.tolist()},
        "gauss_fall_long_wf": {"type": "arbitrary", "samples": gauss_fall_long_wf.tolist()},
        **{f"gauss_flattop_{flat_ns}_wf": {"type": "arbitrary", "samples": wf.tolist()} for flat_ns, wf in gauss_flattop_wfs.items()},
