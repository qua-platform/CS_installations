...
for rr in resonators:
    config["elements"][rr]["time_of_flight"] = 300


for rr in resonators:
    con, fem, port = rr.split("_")[1].split("-")
    for fem, fem_val in config["controllers"][con]["fems"].items():
        for port, port_val in fem_val["analog_inputs"].items():
            port_val["gain_db"] = 0
...
fig, axss = plt.subplots(ncols, nrows, figsize=(3 * nrows, 3 * ncols), sharex=True)
