...
This procedure conducts a broad 1D frequency sweep of the resonator, measuring it while sweeping an
...
Prerequisites:
    ...
    -Calibration of the IQ mixer connected to the resonator drive line (be it an external mixer or an Octave port).

Before proceeding to the next node:
    -Adjust the resonator frequency settings, labeled as "resonator_IF" and "resonator_LO", in the configuration.
"""
...
from octave_tools import octave_calibration, set_correction_parameters
...
# The LO frequency sweep parameters (better performances if LO freq is a multiple of 250MHz)
f_min_external = 3e9
f_max_external = 4e9
...
with program() as resonator_spec:
    ...
    f = declare(int)  # QUA variable for the resonator frequency
    ...
    with for_(i, 0, i < len(freqs_external) + 1, i + 1):
        ...
        with for_(n, 0, n < n_avg, n + 1):
            with for_(*from_array(f, frequencies)):
                # Update the frequency of the digital oscillator linked to the FTR element
                ...
                # Wait for the resonator to decay to the ground state
...
# Calibrate the Octave port
octave_calibration(qm, "FTR", freqs_external, frequencies[len(frequencies)//2])
...
for i in range(len(freqs_external)):  # Loop over the LO frequencies
    ...
    qm.octave.set_lo_frequency("FTR", freqs_external[i])
    qm.octave.set_rf_output_gain("FTR", 0)  # Set the gain again after setting the LO frequency
    set_correction_parameters("", config, "FTR", freqs_external[i], frequencies[len(frequencies)//2], 0, qm)
    # Simpler method to update the correction parameters for the current LO frequency and gain, and IF defined in the config
    # qm.octave.set_element_parameters_from_calibration_db("FTR", job)
