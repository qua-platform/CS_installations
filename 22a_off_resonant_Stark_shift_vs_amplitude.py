...
with program() as prog:
    ...
    with stream_processing():
        ...
        for ind, rr in enumerate(resonators):
            I_st[ind].buffer(len(ts_cycle)).buffer(len(amps)).average().save(f"I_{rr}")
            Q_st[ind].buffer(len(ts_cycle)).buffer(len(amps)).average().save(f"Q_{rr}")
    ...
    if simulate:
        ...
        try:
            ...
            while results.is_processing():
                ...
                for ax, Vname, V in zip(axss.T.ravel(), fetch_names[1:], res[1:]):
            ...
            for i, amp in enumerate(amps):
                try:
                    ...
                    plt.title(f"Ramsey with off-resonant drive at amp = {amp}")
                except (Exception,):
                    pass
                finally:
                    save_data_dict.update({f"fig_analysis_target_{i:03d}_amp={amp:4.3f}": fig_analysis})

            fig_summary = plt.figure()
            plt.plot(amps, detuning_qt)
            plt.xlabel("Amplitudes scale")
