...
node = QualibrationNode[Parameters, Quam](
    name="03c_qubit_spectroscopy_vs_flux_fixed",
...
def create_qua_program(node: QualibrationNode[Parameters, Quam]):
    ...
    if not node.parameters.frequency_center_in_mhz:
        dfs = np.arange(-span / 2, +span / 2, step)
    else:
        dfs = np.arange(-span/2+node.parameters.frequency_center_in_mhz * u.MHz, span/2+node.parameters.frequency_center_in_mhz * u.MHz, step)

    # Flux bias sweep in V
    span = node.parameters.flux_offset_span_in_v * u.V
    num = node.parameters.num_flux_points
    if not node.parameters.flux_center_in_v:
        dcs = np.linspace(-span / 2, +span / 2, num)
    else:
        dcs = np.linspace(-span/2+node.parameters.flux_center_in_v, span/2+node.parameters.flux_center_in_v, num)
