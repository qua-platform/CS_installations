...
saturation_amp = (
    0.005  # Pre-factor to the value defined in the config - restricted to [-2; 2)
)
# Adjust the pulse amplitude and frequency to the storage cavity
off_saturation_amp = (
    1  # Pre-factor to the value defined in the config - restricted to [-2; 2)
)
...
with program() as qubit_AC_stark_shift:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(*from_array(df, dfs)):
            ...
            play(
                "off_pump" * amp(off_saturation_amp),
                "resonator",
            ...
            state, I, Q = macros.readout_macro(
                threshold=ge_threshold, state=state, I=I, Q=Q
            )
...
if simulate:
    ...
    results = fetching_tool(
        job, data_list=["I", "Q", "state", "iteration"], mode="live"
    )
    # Live plotting
    ...
    while results.is_processing():
        ...
        # Plot results
        plt.suptitle(
            f"Qubit AC Start Shift due to readout pumping - LO = {qubit_LO / u.GHz} GHz"
        )
        plt.subplot(211)
        plt.cla()
