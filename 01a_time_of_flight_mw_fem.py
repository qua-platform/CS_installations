...
with program() as raw_trace_prog:
    ...
    with stream_processing():
        ...
        # Will save average:
        adc_st.input1().real().average().save(f"adcI")
        adc_st.input1().image().average().save(f"adcQ")
        # Will save only last run:
        adc_st.input1().real().save(f"adc_single_runI")
        adc_st.input1().image().save(f"adc_single_runQ")
