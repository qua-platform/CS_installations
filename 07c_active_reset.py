...
# initialization_method = "nothing"
initialization_method = "active_reset_fast"  # "thermalization", "active_reset_one_threshold", "active_reset_two_thresholds", "active_reset_fast"
...
###################################
def qubit_initialization(method: str, I, Q):
    """
    ...
    if method == "nothing":
        ...
        return active_reset_fast(ge_threshold_e, I, Q)
    elif method == "active_reset_one_threshold":
        return active_reset_one_threshold(ge_threshold_e, max_tries, I, Q)
    elif method == "active_reset_two_thresholds":
        return active_reset_two_thresholds(
            ge_threshold_g, ge_threshold_e, max_tries, I, Q
        )
    else:
        raise ValueError(f"method {method} is not implemented.")


def active_reset_one_threshold(threshold_g: float, max_tries: int, I, Q):
    """
    ...
    """
    counter = declare(int)
    assign(I, threshold_g)
    assign(counter, 0)
    align("resonator", "qubit")
    with while_((I >= threshold_g) & (counter < max_tries)):
...
def active_reset_two_thresholds(
    threshold_g: float, threshold_e: float, max_tries: int, I, Q
):
    """
    ...
    with while_((I >= threshold_g) & (counter < max_tries)):
        ...
        wait(depletion_time * u.ns, "qubit")
        # Play a conditional pi-pulse to actively reset the qubit
        play("x180", "qubit", condition=(I > threshold_e))
        # Update the counter for benchmarking purposes
        assign(counter, counter + 1)
    return counter


def active_reset_fast(threshold_g: float, I, Q):
    """
    Active reset protocol where the outcome of the measurement is compared to a pre-calibrated threshold (IQ_blobs.py).
...
with program() as active_reset_prog:
    ...
    with for_(n, 0, n < n_shot, n + 1):
        # Active reset
        count = qubit_initialization(initialization_method, I_g, Q_g)
        align()
        ...
        # Active reset
        count = qubit_initialization(initialization_method, I_e, Q_e)
        align()
        ...
        # Just thermalize the qubit between iterations, the I, Q values are not saved
        qubit_initialization("thermalization", I_e, Q_e)
...
qmm = QuantumMachinesManager(
    host=qop_ip, port=qop_port, port=qop_port, cluster_name=cluster_name, octave=octave_config
)
...
if simulation:
    ...
    print(f"Original {ge_threshold_g=}, {ge_threshold_e=}\n")
    e_to_g_pct = np.count_nonzero(Ie < ge_threshold_e) / len(Ie)
    print(f"|e> to |g> reset infidelity: {1 - e_to_g_pct:.2%}")
    # Save results
    ...
    save_data_dict.update(
        {"two_state_discriminator": [angle, threshold, fidelity, gg, ge, eg, ee]}
    )
