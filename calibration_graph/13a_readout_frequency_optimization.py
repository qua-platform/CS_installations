...
fit_results = {q.name : detuning.loc[q.name].values for q in qubits}
data['fit_results'] = fit_results

for q in qubits:
    print(f"{q.name}: Shifting readout frequency by {fit_results[q.name]/1e3:.0f} KHz")
# %%
...
# %%
for q in qubits:
    q.resonator.intermediate_frequency += float(fit_results[q.name])
# %%
node_save(machine, "readout_frequency_optimization", data, additional_files=True)
