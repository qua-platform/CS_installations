...
node = QualibrationNode(name="04_Power_Rabi_simpler", parameters=Parameters())
...
with program() as power_rabi:
    ...
    a = declare(fixed)  # QUA variable for the qubit frequency
    ...
    with stream_processing():
        ...
        for i in range(num_qubits):
            I_st[i].buffer(len(amps)).buffer(1).average().save(f"I{i + 1}")
            Q_st[i].buffer(len(amps)).buffer(1).average().save(f"Q{i + 1}")
