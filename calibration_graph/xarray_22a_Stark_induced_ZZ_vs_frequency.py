...
class Parameters(NodeParameters):
    ...
    zz_control_phases: List[float] = [0.5]
...
delta_phase = 4e-9 * 1e6 * node.parameters.freq_detuning_in_mhz * node.parameters.wait_time_step_in_ns
detuning = int(1e6 * node.parameters.freq_detuning_in_mhz)
...
with program() as stark_induced_zz_vs_frequency:
    ...
    for i, qp in enumerate(qubit_pairs):
        ...
        qc = qp.qubit_control
        qt = qp.qubit_target
        qt_xyd.intermediate_frequency = zz.intermediate_frequency
    ...
    with stream_processing():
        ...
        for i, qb in enumerate(qubit_pairs):
            # if node.parameters.use_state_discrimination:
            #     state_st_control[i].buffer(2).buffer(len(idle_time_cycles)).buffer(len(dfs)).average().save(f"state_control{i + 1}")
            #     state_st_target[i].buffer(2).buffer(len(idle_time_cycles)).buffer(len(dfs)).average().save(f"state_target{i + 1}")
            # else:
...
if node.parameters.simulate:
        ...
        for i, (s, St) in enumerate(zip(["g", "e"], [St_g, St_e])):
            for j, df in enumerate(dfs):
                try:
                    ...
                    detuning_qt[i, j] = ramsey_fit["f"][0] * u.GHz - node.parameters.freq_detuning_in_mhz * u.MHz
    ...
    # fit = fit_oscillation_decay_exp(ds.I_target, "time")
    ...
    # within_detuning = (1e9 * frequency < 2 * detuning).mean(dim="qc_state") == 1
    # positive_shift = frequency.sel(qc_state=1) > frequency.sel(qc_state=0)
    # freq_offset = (
    #     within_detuning * (frequency * fit.qc_state).mean(dim="qc_state")
    #     + ~within_detuning * positive_shift * frequency.mean(dim="qc_state")
    #     - ~within_detuning * ~positive_shift * frequency.mean(dim="qc_state")
    # )
    # decay = 1e-9 * tau.mean(dim="qc_state")
    # decay_error = 1e-9 * tau_error.mean(dim="qc_state")

    # # Save fitting results
    # fit_results = {
    #     qp.name: {
    #         "freq_offset": 1e9 * freq_offset.loc[qp.name].values,
    #         "decay": decay.loc[qp.name].values,
    #         "decay_error": decay_error.loc[qp.name].values,
    #     }
    #     for qp in qubit_pairs
    # }
    ...
    # # %% {Plotting}
    # grid_names = [f"{qp.name}_0" for qp in qubit_pairs]
    # grid = QubitGrid(ds, grid_names)
    # for ax, qubit in grid_iter(grid):
    #     if node.parameters.use_state_discrimination:
    #         ds.sel(qc_state=1).loc[qubit].state.plot(
    #             ax=ax, x="time", c="C0", marker=".", ms=5.0, ls="", label="$\Delta$ = +"
    #         )
    #         ds.sel(qc_state=0).loc[qubit].state.plot(
    #             ax=ax, x="time", c="C1", marker=".", ms=5.0, ls="", label="$\Delta$ = -"
    #         )
    #         ax.plot(ds.time, fitted.loc[qubit].sel(qc_state=1), c="C0", ls="-", lw=1)
    #         ax.plot(ds.time, fitted.loc[qubit].sel(qc_state=0), c="C1", ls="-", lw=1)
    #         ax.set_ylabel("State")
    #     else:
    #         (ds.sel(qc_state=1).loc[qubit].I_target * 1e3).plot(
    #             ax=ax, x="time", c="C0", marker=".", ms=5.0, ls="", label="$\Delta$ = +"
    #         )
    #         (ds.sel(qc_state=0).loc[qubit].I_target * 1e3).plot(
    #             ax=ax, x="time", c="C1", marker=".", ms=5.0, ls="", label="$\Delta$ = -"
    #         )
    #         ax.set_ylabel("Trans. amp. I [mV]")
    #         ax.plot(ds.time, 1e3 * fitted.loc[qubit].sel(qc_state=1), c="C0", ls="-", lw=1)
    #         ax.plot(ds.time, 1e3 * fitted.loc[qubit].sel(qc_state=0), c="C1", ls="-", lw=1)
...
if not node.parameters.simulate:
    with node.record_state_updates():
        zz_detunings_in_mhz = [-11]
        for i, qp in enumerate(qubit_pairs):
            qp.zz_drive.detuning = zz_detunings_in_mhz[i] * u.MHz
