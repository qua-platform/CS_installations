...
with program() as stark_detuning:
    ...
    with stream_processing():
        n_st.save("n")
        for i, qubit in enumerate(qubits):
            state_stream[i].boolean_to_int().buffer(N_pi).average().save(f"state{i + 1}")
            I_stream = I_st[i].buffer(N_pi).average().save(f"I{i + 1}")
            Q_stream = Q_st[i].buffer(N_pi).average().save(f"Q{i + 1}")
...
if node.parameters.simulate:
    ...
    # plt.figure()
    # plt.plot(config["waveforms"]["q7.xy.x180_DragCosine.wf.I"]["samples"])
    # plt.plot(config["waveforms"]["q7.xy.x180_DragCosine.wf.Q"]["samples"])
    # plt.show()
    machine.qubits["q7"].xy.operations["x180"].alpha = -00.0
    for det in detunings:
        xaxis.append(det)
        # Derive the DRAG waveforms with the new detuning
        machine.qubits["q7"].xy.operations["x180"].detuning = det
        config = machine.generate_config()
        ...
        results = fetching_tool(job, data_list=["I1", "Q1", "state1"])
        # Fetch results
        I, Q, state = results.fetch_all()
        # Convert the results into Volts
        I, Q = u.demod2volts(I, 2000), u.demod2volts(Q, 2000)
        I_tot.append(I)
        Q_tot.append(Q)
        ...
        plt.subplot(231)
        plt.cla()
        plt.pcolor(N_pi_vec, xaxis, I_tot)
        plt.xlabel("# of x180-x180 pulses")
        plt.ylabel("Detuning [Hz]")
        plt.title("I [V]")
        plt.subplot(232)
        plt.cla()
        plt.pcolor(N_pi_vec, xaxis, Q_tot)
        plt.xlabel("# of x180-x180 pulses")
        plt.ylabel("Detuning [Hz]")
        plt.title("Q [V]")
        plt.subplot(233)
        plt.cla()
        plt.pcolor(N_pi_vec, xaxis, state_tot)
        plt.xlabel("# of x180-x180 pulses")
        plt.ylabel("Detuning [Hz]")
        plt.title("state")
        plt.subplot(212)
