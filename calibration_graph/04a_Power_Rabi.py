...
with program() as power_rabi:
    ...
    with stream_processing():
        ...
        for j, q in enumerate(qubits):
            if operation == "x180":
                if state_discrimination:
                    state_st[j].boolean_to_int().buffer(len(amps)).buffer(np.ceil(N_pi / 2)).average().save(f"state{j + 1}")
                else:
                ...
                if state_discrimination:
                    state_st[j].boolean_to_int().buffer(len(amps)).buffer(np.ceil(N_pi / 4)).average().save(f"state{j + 1}")
                else:
