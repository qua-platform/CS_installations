...
ds = fetch_results_as_xarray(handles, qubits, {"N": np.linspace(0, n_runs// 2, n_runs // 2)})
...
for ax, qubit in grid_iter(grid):
    ax.plot(ds.I_g.loc[qubit], ds.Q_g.loc[qubit], label='|g>',marker = '.',markersize= 0.3,ls = "")
    ax.plot(ds.I_e.loc[qubit], ds.Q_e.loc[qubit], label='|e>',marker = '.',markersize= 0.3,ls = "")
plt.show()
