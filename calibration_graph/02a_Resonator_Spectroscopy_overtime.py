...
node = QualibrationNode(name="02a_Resonator_Spectroscopy_overtime", parameters=Parameters())
assert not (
...
with program() as multi_res_spec:
    ...
    k = declare(int)  # QUA variable for the qubit frequency

    # Bring the active qubits to the minimum frequency point
    machine.apply_all_flux_to_min()

    for i, rr in enumerate(resonators):
        with for_(k, 0, k < N_repeat, k + 1):
            save(k, n_st)
            rr.wait(1 * u.s)
    ...
    with stream_processing():
        ...
        for i in range(num_qubits):
            I_st[i].buffer(len(dfs)).buffer(n_avg).map(FUNCTIONS.average()).buffer(N_repeat).save(f"I{i + 1}")
            Q_st[i].buffer(len(dfs)).buffer(n_avg).map(FUNCTIONS.average()).buffer(N_repeat).save(f"Q{i + 1}")
...
if node.parameters.simulate:
    ...
    with qm_session(qmm, config, timeout=node.parameters.timeout) as qm:
        ...
        while results.is_processing():
            ...
            # Progress bar
            progress_counter(n, N_repeat, start_time=results.start_time)
...
if not node.parameters.simulate:
    ...
    if node.parameters.load_data_id is not None:
        ...
        ds = fetch_results_as_xarray(job.result_handles, qubits, {"freq": dfs, "N": 1.2*np.linspace(1, N_repeat, N_repeat)})
    ...
    for ax, qubit in grid_iter(grid):
        (ds.loc[qubit].IQ_abs * 1e3).plot(ax=ax)
    ...
    grid = QubitGrid(ds, [q.grid_location for q in qubits])
    for ax, qubit in grid_iter(grid):
        (ds.loc[qubit].phase * 1e3).plot(ax=ax)
        ax.set_xlabel("Resonator detuning [MHz]")
        ax.set_ylabel("Time [s]")
        ax.set_title(qubit["qubit"])
    grid.fig.suptitle("Resonator spectroscopy (raw data)")
