...
with program() as iq_blobs:
    ...
    with stream_processing():
        ...
        for i in range(num_qubits):
            I_g_st[i].buffer(len(amps)).buffer(len(dfs)).buffer(n_runs).save(f"I_g{i + 1}")
            Q_g_st[i].buffer(len(amps)).buffer(len(dfs)).buffer(n_runs).save(f"Q_g{i + 1}")
            I_e_st[i].buffer(len(amps)).buffer(len(dfs)).buffer(n_runs).save(f"I_e{i + 1}")
            Q_e_st[i].buffer(len(amps)).buffer(len(dfs)).buffer(n_runs).save(f"Q_e{i + 1}")
...
if not node.parameters.simulate:
    if node.parameters.load_data_id is None:
        ...
        ds = fetch_results_as_xarray(job.result_handles, qubits, {"amplitude": amps, "freq": dfs, "N": np.linspace(1, n_runs, n_runs)})
    ...
    best_amp = {}
    for q in qubits:
        best_amp = float(fit_res.sel(qubit=q.name, result="meas_fidelity").max(dim="freq").idxmax(dim="amplitude"))
        best_freq = float(fit_res.sel(qubit=q.name, result="meas_fidelity").max(dim="amplitude").idxmax(dim="freq"))
        best_fid = float(
            fit_res.sel(qubit=q.name, result="meas_fidelity").sel(amplitude=best_amp, freq=best_freq).values)
        print(f"Best readout fidelity:{best_fid} for amp={best_amp} and freq={best_freq}")
        plt.figure()
        fit_res.sel(qubit=q.name, result="meas_fidelity").plot()
        plt.show()
        fig = plt.figure()
        amplitude = best_amp
        frequency = best_freq
        ds_q = ds.sel(qubit=q.name, amplitude=amplitude, freq=frequency)
