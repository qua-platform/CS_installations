...
# %%
if not node.parameters.simulate:
    ds_control = ds_control.assign_coords(idle_time=4*ds_control.idle_time/1e3)  # convert to usec
    ...
    fitvals_control = frequency_control.polyfit(dim = 'flux', deg = 2)
    flux_control = frequency_control.flux
    a_control = {}
    flux_offset_control = {}
    freq_offset_control = {}
    for q in qubit_pairs:
        a_control[q.name] = float(-1e6*fitvals_control.sel(qubit = q.name,degree = 2).polyfit_coefficients.values)
        flux_offset_control[q.name]  = float((-0.5*fitvals_control.sel(qubit =q.name,degree = 1).polyfit_coefficients/fitvals_control.sel(qubit = q.name,degree = 2).polyfit_coefficients).values)
        freq_offset_control[q.name]  = 1e6*float(fitvals_control.sel(qubit = q.name,degree = 0).polyfit_coefficients.values) - detuning



    ds_target = ds_target.assign_coords(idle_time=4*ds_target.idle_time/1e3)  # convert to usec
    ds_target.flux.attrs = {'long_name': 'flux', 'units': 'V'}
    ds_target.idle_time.attrs = {'long_name': 'idle time', 'units': 'usec'}
    ...
    fitvals_target = frequency_target.polyfit(dim = 'flux', deg = 2)
    flux_target = frequency_target.flux
    a_target = {}
    flux_offset_target = {}
    freq_offset_target = {}
    for q in qubit_pairs:
        a_target[q.name] = float(-1e6*fitvals_target.sel(qubit = q.name,degree = 2).polyfit_coefficients.values)
        flux_offset_target[q.name]  = float((-0.5*fitvals_target.sel(qubit =q.name,degree = 1).polyfit_coefficients/fitvals_target.sel(qubit = q.name,degree = 2).polyfit_coefficients).values)
        freq_offset_target[q.name]  = 1e6*float(fitvals_target.sel(qubit = q.name,degree = 0).polyfit_coefficients.values) - detuning
...
if not node.parameters.simulate:
    ...
    grid = QubitPairGrid(grid_names, qubit_pair_names)
    for ax, qubit in grid_iter(grid):
        fitted_freq = fitvals_control.sel(qubit = qubit['qubit'],degree = 2).polyfit_coefficients * ds.flux**2 + fitvals_control.sel(qubit = qubit['qubit'],degree = 1).polyfit_coefficients * ds.flux + fitvals_control.sel(qubit = qubit['qubit'],degree = 0).polyfit_coefficients
        frequency_control.sel(qubit = qubit['qubit']).plot( marker = '.',linewidth = 0,ax=ax)
        ax.plot(ds.flux,fitted_freq)
        ax.set_title(qubit['qubit'])
        ax.set_xlabel(' Flux (V)')
        print(f"The quad term for {qubit['qubit']} is {a_control[qubit['qubit']]/1e9:.3f} GHz/V^2")
        print(f"Flux offset for {qubit['qubit']} is {flux_offset_control[qubit['qubit']]*1e3:.1f} mV")
        print(f"Freq offset for {qubit['qubit']} is {freq_offset_control[qubit['qubit']]/1e6:.3f} MHz")
        print()
    grid.fig.suptitle('Ramsey freq. Vs. flux control')
    plt.tight_layout()
    plt.show()
    ...
    grid = QubitPairGrid(grid_names, qubit_pair_names)
    for ax, qubit in grid_iter(grid):
        fitted_freq = fitvals_target.sel(qubit = qubit['qubit'],degree = 2).polyfit_coefficients * ds_target.flux**2 + fitvals_target.sel(qubit = qubit['qubit'],degree = 1).polyfit_coefficients * ds_target.flux + fitvals_target.sel(qubit = qubit['qubit'],degree = 0).polyfit_coefficients
        frequency_target.sel(qubit = qubit['qubit']).plot( marker = '.',linewidth = 0,ax=ax)
        ax.plot(ds_target.flux,fitted_freq)
        ax.set_title(qubit['qubit'])
        ax.set_xlabel(' Flux (V)')
        print(f"The quad term for {qubit['qubit']} is {a_target[qubit['qubit']]/1e9:.3f} GHz/V^2")
        print(f"Flux offset for {qubit['qubit']} is {flux_offset_target[qubit['qubit']]*1e3:.1f} mV")
        print(f"Freq offset for {qubit['qubit']} is {freq_offset_target[qubit['qubit']]/1e6:.3f} MHz")
        print()
    grid.fig.suptitle('Ramsey freq. Vs. flux target')
    plt.tight_layout()
    plt.show()
...
if not node.parameters.simulate:
    ...
    for q in qubit_pairs:
        ...
        node.results['fit_results'][q.name]['flux_offset_control'] = flux_offset_control[q.name]
        node.results['fit_results'][q.name]['freq_offset_control'] = freq_offset_control[q.name]
        node.results['fit_results'][q.name]['quad_term_control'] = a_control[q.name]
        node.results['fit_results'][q.name]['flux_offset_target'] = flux_offset_target[q.name]
        node.results['fit_results'][q.name]['freq_offset_target'] = freq_offset_target[q.name]
        node.results['fit_results'][q.name]['quad_term_target'] = a_target[q.name]
        
# %% {Update_state}
if not node.parameters.simulate:
    with node.record_state_updates():
        for qp in qubit_pairs:
            qp.qubit_control.xy.intermediate_frequency -= freq_offset_control[qp.name]
            qp.qubit_target.xy.intermediate_frequency -= freq_offset_target[qp.name]
            qp.mutual_flux_bias[0] += flux_offset_control[qp.name]
            qp.mutual_flux_bias[1] += flux_offset_target[qp.name]
            qp.qubit_control.freq_vs_flux_01_quad_term = float(a_control[qp.name])
            qp.qubit_target.freq_vs_flux_01_quad_term = float(a_target[qp.name])
# %% {Save_results}
if not node.parameters.simulate:    
