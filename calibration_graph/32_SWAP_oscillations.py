...
node = QualibrationNode(
    name="32_SWAP_oscillations", parameters=Parameters()
)
assert not (
...
with program() as swap_oscillations:
        ...
        with for_(n, 0, n < n_avg, n + 1):
            ...
            with for_(*from_array(amp, amplitudes)):
                ...
                with for_(*from_array(t, times_cycles)):
                    ...
                    # play the flux pulse
                    with if_(t > 0):
                        qp.qubit_control.z.play('const', duration=t, amplitude_scale=pulse_amplitudes[qp.name] /
                                                                                     qp.qubit_control.z.operations[
                                                                                         'const'].amplitude * amp)
...
if node.parameters.simulate:
    ...
    job = qmm.simulate(config, swap_oscillations, simulation_config)
    ...
    job = qm.execute(swap_oscillations)
...
if not node.parameters.simulate:
    ...
    for ax, qubit_pair in grid_iter(grid):
        plot = ds.assign_coords(detuning_MHz=1e-6 * ds.detuning).state_control.sel(qubit=qubit_pair['qubit']).plot(
            ax=ax, x='time', y='detuning_MHz', add_colorbar=False)
        plt.colorbar(plot, ax=ax, orientation='horizontal', pad=0.2, aspect=30, label='Amplitude')
        ax.plot([lengths[qubit_pair['qubit']] - zero_paddings[qubit_pair['qubit']]],
                [1e-6 * detunings[qubit_pair['qubit']]], marker='.', color='red')
        ax.axhline(y=1e-6 * detunings[qubit_pair['qubit']], color='k', linestyle='--', lw=0.5)
        ax.axvline(x=lengths[qubit_pair['qubit']] - zero_paddings[qubit_pair['qubit']], color='k', linestyle='--',
                   lw=0.5)
    ...
    grid = QubitPairGrid(grid_names, qubit_pair_names)
    for ax, qubit_pair in grid_iter(grid):
        plot = ds.assign_coords(detuning_MHz=1e-6 * ds.detuning).state_target.sel(qubit=qubit_pair['qubit']).plot(ax=ax,
                                                                                                                  x='time',
                                                                                                                  y='detuning_MHz',
                                                                                                                  add_colorbar=False)
        plt.colorbar(plot, ax=ax, orientation='horizontal', pad=0.2, aspect=30, label='Amplitude')
        ...
        with node.record_state_updates():
            for qp in qubit_pairs:
                qp.gates['SWAP_unipolar'] = CZGate(
                    flux_pulse_control=FluxPulse(length=lengths[qp.name], amplitude=amplitudes[qp.name] / 2,
