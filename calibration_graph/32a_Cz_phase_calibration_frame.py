...
4. Measuring the population in the |f> state to quantify leakage -- not implemented yet
...
class Parameters(NodeParameters):
    ...
    amp_step : float = 0.1
    num_frames: int = 10
    load_data_id: Optional[int] = 92417 
    plot_raw : bool = False
...
with program() as CPhase_Oscillations:
    ...
    with stream_processing():
        ...
        for i in range(num_qubit_pairs):
            state_st_control[i].buffer(len(amplitudes)).buffer(len(frames)).buffer(2).average().save(f"state_control{i + 1}")
            state_st_target[i].buffer(len(amplitudes)).buffer(len(frames)).buffer(2).average().save(f"state_target{i + 1}")
