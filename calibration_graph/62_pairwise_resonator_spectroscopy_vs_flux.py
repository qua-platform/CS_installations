...
node = QualibrationNode(
    name="62_pairwise_resonator_spectroscopy_vs_flux", parameters=Parameters()
)
...
with program() as multi_res_spec_vs_flux:
    ...
    state_stream = [declare_stream() for _ in range(num_qubit_pairs)]
    dc = declare(fixed)  # QUA variable for the flux bias
    t = declare(int)  # QUA variable for the readout frequency

    for i, qp in enumerate(qubit_pairs):

        detuning = qp.qubit_target.xy.intermediate_frequency - qp.qubit_control.xy.intermediate_frequency
        control_flux_offset = np.sqrt(detuning / qp.qubit_control.freq_vs_flux_01_quad_term)
        ...
        wait(1000)
        qp.qubit_target.z.set_dc_offset(qp.qubit_target.z.joint_offset + control_flux_offset)
            ...
            with for_(*from_array(dc, dcs)):
                ...
                with for_(*from_array(t, idle_times)):
                    ...
                    if node.parameters.reset_type == "active":
                        active_reset(machine, qp.qubit_target.name)
                        active_reset(machine, qp.qubit_control.name)
                    ...
                    qp.qubit_target.xy.play("x180", duration=t)
                    
                    align()
                    qp.qubit_control.resonator.measure("readout", qua_vars=(I1[i], Q1[i]))
                    assign(
                        state[i], I1[i] > qp.qubit_target.resonator.operations["readout"].threshold
                    )
    ...
    with stream_processing():
        ...
        for i in range(num_qubit_pairs):
            state_stream[i].boolean_to_int().buffer(len(idle_times)).buffer(len(dcs)).average().save(f"state{i + 1}")
            I1_st[i].buffer(len(idle_times)).buffer(len(dcs)).average().save(f"I{i + 1}")
            Q1_st[i].buffer(len(idle_times)).buffer(len(dcs)).average().save(f"Q{i + 1}")
...
if node.parameters.simulate:
    ...
    ds = fetch_results_as_xarray(job.result_handles, qubit_pairs, {"idle_times": idle_times, "flux": dcs})
    # Derive the amplitude IQ_abs = sqrt(I**2 + Q**2)
    ds = ds.assign({"IQ_abs1": np.sqrt(ds["state"] ** 2 + ds["Qcontrol"] ** 2)})
