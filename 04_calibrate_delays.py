...
Next steps before going to the next node:
    ...
    - Update the delay between the laser and mw (mw_delay) and the mw length (mw_switch_len) in the configuration.
...
laser_delay = 100  # delay before laser [ns]
initialization_len = 8_000  # laser duration length [ns]
mw_switch_len = 1_000  # MW duration length [ns]
...
ts_cycle = np.arange(0, meas_len, 1)

assert (initialization_len - mw_switch_len) > 4, "The MW must be shorter than the laser pulse"
...
def calibrate_delays(
    ...
    if mw_on:
        wait((laser_delay + (initialization_len - mw_switch_len) // 2) * u.ns, "MW_Switch")
        play("cw", "NV", duration=mw_switch_len * u.ns)
...
if simulate:
    ...
    while results.is_processing():
        ...
        plt.cla()
        plt.plot(ts[::resolution] + resolution / 2, times_on_hist / 1000 / (resolution / u.s) / iterations)
        plt.plot(ts[::resolution] + resolution / 2, times_off_hist / 1000 / (resolution / u.s) / iterations)
        plt.xlabel("t [ns]")
