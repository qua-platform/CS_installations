...
The "qua.qua.demod.sliced" method is employed to capture the time trace of the qua.qua.demodulated data, providing insight into the
resonator's response.
Reference: https://docs.quantum-machines.co/0.1/qm-qua-sdk/docs/Guides/features/?h=accumulated#sliced-qua.qua.demodulation
...
readout_len = 15 * u.us  # Readout pulse duration
ringdown_len = 15 * u.us  # integration time after readout pulse to observe the ringdown of the resonator
...
with qua.program() as opt_weights:
    n = qua.declare(int)
    ind = qua.declare(int)
    II = qua.declare(qua.fixed, size=number_of_divisions)
    IQ = qua.declare(qua.fixed, size=number_of_divisions)
    QI = qua.declare(qua.fixed, size=number_of_divisions)
    QQ = qua.declare(qua.fixed, size=number_of_divisions)

    n_st = qua.declare_stream()
    II_st = qua.declare_stream()
    IQ_st = qua.declare_stream()
    QI_st = qua.declare_stream()
    QQ_st = qua.declare_stream()
    ...
    with qua.for_(n, 0, n < n_avg, n + 1):
        ...
        with qua.for_(ind, 0, ind < number_of_divisions, ind + 1):
            qua.save(II[ind], II_st)
            qua.save(IQ[ind], IQ_st)
            qua.save(QI[ind], QI_st)
            qua.save(QQ[ind], QQ_st)

        qua.align()  # Global qua.align to qua.play the pi pulse after thermalization
        ...
        with qua.for_(ind, 0, ind < number_of_divisions, ind + 1):
            ...
            qua.save(QI[ind], QI_st)
            qua.save(QQ[ind], QQ_st)
            qua.wait(10 * u.us, 'qubit')
            qua.play('x180'*qua.amp(0), 'qubit')
        qua.save(n, n_st)
...
if simulate:
    ...
    # Sum the quadrature to fully qua.qua.demodulate the traces
    Ie = IIe + IQe
    Ig = IIg + IQg
    Qe = QIe + QQe
    Qg = QIg + QQg
    ...
    # And finally extract the weights from the qua.saved file and reformat them using the integration_weights_tools.
