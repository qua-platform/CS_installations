...
with program() as prog:
    ...
    with stream_processing():
        ...
        for ind, rr in enumerate(resonators):
            I_st[ind].buffer(2).buffer(len(ts_cycles)).buffer(len(amps)).average().save(f"I_{rr}")
            Q_st[ind].buffer(2).buffer(len(ts_cycles)).buffer(len(amps)).average().save(f"Q_{rr}")
