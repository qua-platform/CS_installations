    ...
    with for_(n, 0, n < n_avg, n + 1):
        ...
        with for_(*from_array(df, dfs)):
            ...
            with for_(*from_array(dc, dcs)):
                # sweeping for all qubits and couplers
                for q, coupler in zip(qubits, couplers):
                    q.to_min()
                    coupler.bias.set_dc_offset(dc)
...
if simulate:
    ...
    while results.is_processing():
        ...
        plt.suptitle("Qubit spectroscopy vs coupler")
        S_data = []
        for i, q in enumerate(qubits):
