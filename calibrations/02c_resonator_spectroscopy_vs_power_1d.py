...
from calibration_utils.resonator_spectroscopy_vs_amplitude_1d import (
    Parameters,
    process_raw_dataset,
    fit_raw_data,
    log_fitted_results,
    plot_raw_data_with_fit,
    plot_raw_data_without_fit
)
...
node = QualibrationNode[Parameters, Quam](
    name="02c_resonator_vs_power",  # Name should be unique
...
def create_qua_program(node: QualibrationNode[Parameters, Quam]):
    ...
    node.namespace["sweep_axes"] = {
        ...
        "amplitude": xr.DataArray(amps, attrs={"long_name": "readout amplitude", "units": "a.u."}),
    }
...
def plot_data(node: QualibrationNode[Parameters, Quam]):
    ...
    fig_raw_fit = plot_raw_data_without_fit(node.results["ds_raw"], node.namespace["qubits"])
    plt.show()
