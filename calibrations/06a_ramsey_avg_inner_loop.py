    ...
    with program() as node.namespace["qua_program"]:
        ...
        with stream_processing():
            ...
            for i in range(num_qubits):
                if node.parameters.use_state_discrimination:
                    state_st[i].buffer(n_avg).map(FUNCTIONS.average()).buffer(len(detuning_signs)).buffer(len(idle_times)).save(f"state{i + 1}")
                else:
                    I_st[i].buffer(n_avg).map(FUNCTIONS.average()).buffer(len(detuning_signs)).buffer(len(idle_times)).save(f"I{i + 1}")
                    Q_st[i].buffer(n_avg).map(FUNCTIONS.average()).buffer(len(detuning_signs)).buffer(len(idle_times)).save(f"Q{i + 1}")
...
def execute_qua_program(node: QualibrationNode[Parameters, Quam]):
    ...
    with qm_session(qmm, config, timeout=node.parameters.timeout) as qm:
        ...
        for dataset in data_fetcher:
            progress_counter(
                data_fetcher["n"],
                len(get_idle_times_in_clock_cycles(node.parameters)), # node.parameters.num_shots,
