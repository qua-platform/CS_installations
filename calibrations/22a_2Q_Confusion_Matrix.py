...
# %%
if not node.parameters.simulate:
    state2 =(ds.sel(qubit=qp.name).state_control * 2 + ds.sel(qubit=qp.name).state_target)
    states = [0, 1, 2, 3]
        ...
        for state in states:
            ...
            for q1 in [0, 1]:
                for q0 in [0, 1]:
                    ...
                    row2.append((state2.sel(init_state_target=q0, init_state_control=q1) == state).sum().values)
            conf.append(row)
            conf2.append(row2)
        confusions[qp.name] = np.array(conf) / node.parameters.num_shots
        confusions2[qp.name] = np.array(conf2) / node.parameters.num_shots
...
if not node.parameters.simulate:
    ...
    for ax, qubit_pair in grid_iter(grid):
        ...
        conf = confusions2[qubit_pair['qubit']]
...
if not node.parameters.simulate:
    if node.parameters.load_data_id is None:
        with node.record_state_updates():
            for qp in qubit_pairs:
                qp.confusion = confusions2[qp.name].tolist()
# %% {Save_results}
if not node.parameters.simulate:
