...
node = QualibrationNode[Parameters, Quam](
    name="02a_wide_resonator_spectroscopy",  # Name should be unique
...
def create_qua_program(node: QualibrationNode[Parameters, Quam]):
    ...
    dfs = np.arange(init_freq, +span, step) # start at 20 MHz and go upwards up to span
            ...
            with for_(n, 0, n < n_avg, n + 1):
                ...
                with for_(*from_array(df, dfs)):
                    for i, qubit in multiplexed_qubits.items():
                        ...
                        rr.update_frequency(df) # starts at 20 MHz up to span
...
def plot_data(node: QualibrationNode[Parameters, Quam]):
    ...
    fig_gradient = plot_gradient(node.results["ds_raw"], node.namespace["qubits"])
