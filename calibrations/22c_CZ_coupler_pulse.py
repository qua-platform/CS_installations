    ...
    with for_(n, 0, n < n_avg, n + 1):
        ...
        with for_(*from_array(t, ts)):
            with for_(*from_array(a, amps)):
                ...
                # q1.z.play("const", amplitude_scale=0.0531 + scale * a * coupler.operations["const"].amplitude, duration=t) # -0.048 
...
if simulate:
    ...
    while results.is_processing():
        ...
        plt.subplot(221)
        plt.cla()
        plt.pcolor(amps * coupler.operations["const"].amplitude, 4 * ts, I1)
        # plt.plot(cz_point, wait_time, color="r", marker="*")
        # plt.title(f"{q1.name} - I, f_01={int(q1.f_01 / u.MHz)} MHz")
        plt.ylabel("Interaction time [ns]")
        plt.subplot(223)
        plt.cla()
        plt.pcolor(amps * coupler.operations["const"].amplitude, 4 * ts, Q1)
        # plt.plot(cz_point, wait_time, color="r", marker="*")
        ...
        plt.subplot(222)
        plt.cla()
        plt.pcolor(amps * coupler.operations["const"].amplitude, 4 * ts, I2)
        # plt.plot(cz_point, wait_time, color="r", marker="*")
        # plt.title(f"{q2.name} - I, f_01={int(q2.f_01 / u.MHz)} MHz")
        plt.subplot(224)
        plt.cla()
        plt.pcolor(amps * coupler.operations["const"].amplitude, 4 * ts, Q2)
        # plt.plot(cz_point, wait_time, color="r", marker="*")
