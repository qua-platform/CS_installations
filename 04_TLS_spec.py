...
# Choice of element for TLS spectroscopy (tls1, tls2, tls3)
TLS_ELEM = "tls1"
...
with program() as qubit_spec:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(*from_array(df, dfs)):
            ...
            update_frequency(TLS_ELEM, df + center)
            # Play the saturation pulse to put the qubit in a mixed state - Can adjust the amplitude on the fly [-2; 2)
            play("saturation" * amp(saturation_amp), TLS_ELEM, duration=saturation_len * u.ns)
...
if simulate:
    ...
    while results.is_processing():
        ...
        # Plot results
        plt.suptitle(f"TLS spectroscopy - LO = {LO_TLS1 / u.GHz} GHz")
        plt.subplot(211)
        plt.cla()
        plt.plot((dfs + center) / u.MHz, R, ".")
        plt.xlabel("TLS intermediate frequency [MHz]")
