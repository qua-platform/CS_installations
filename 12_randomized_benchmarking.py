...
from qm.qua import *
from qm import QuantumMachinesManager, CompilerOptionArguments
        ...
        with for_(depth, 1, depth <= max_circuit_depth, depth + 1):
            ...
            with if_((depth == 1) | (depth == depth_target)):
                ...
                with for_(n, 0, n < n_avg, n + 1):
                    ...
                    play("laser_ON", "AOM")
                    measure("readout", "APD", None, time_tagging.analog(times, meas_len_1, counts))
...
if simulate:
    ...
    # change the flag for strict timing, that means that when there is a gap in strict timing block, it will raise a
    # warning instead of an error
    my_compiler_options = CompilerOptionArguments(flags=['not-strict-timing'])
    # Send the QUA program to the OPX, which compiles and executes it
    job = qm.execute(rb, compiler_options=my_compiler_options)
    ...
    # Save results
    analysis ={
        "Error_rate": np.format_float_scientific(one_minus_p, precision=2),
        "Error_rate_stdevs": stdevs[2],
        "Clifford_set_infidelity": np.format_float_scientific(r_c, precision=2),
        "Clifford_set_infidelity_Error_rate_stdevs": r_c_std,
        "Gate_infidelity": np.format_float_scientific(r_g, precision=2),
        "Gate_infidelity_stdevs": r_g_std,
    }
    script_name = Path(__file__).name
    data_handler = DataHandler(root_data_folder=save_dir)
    save_data_dict.update({"fig_live": fig})
    save_data_dict.update({"analysis": analysis})
    data_handler.additional_files = {script_name: script_name}
    data_handler.save_data(data=save_data_dict, name="randomized_benchmarking")