...
amps = np.arange(0.80, 1.95, 0.5) # scaling factor for amplitude
...
with program() as PROGRAM:
            ...
            with for_(*from_array(t, ts_cycles)):
                with for_(c, 0, c < 3, c + 1): # bases 
                    with for_(s, 0, s < 2, s + 1): # states
                        ...
                        if cr_type == "direct":
                            align(qc_xy, cr_drive)
                            play("square_positive" * amp(a), cr_drive, duration=t)
                            ...
                            align(qc_xy, cr_drive, cr_cancel)
                            play("square_positive" * amp(a), cr_drive, duration=t)
                            play("square_positive" * amp(a * cr_cancel_amp), cr_cancel, duration=t)
                            # pi pulse on control
                            align(qc_xy, cr_drive, cr_cancel)
                            play("x180", qc_xy)
                            # echoed direct + cancel
                            align(qc_xy, cr_drive, cr_cancel)
                            play("square_negative" * amp(a), cr_drive, duration=t)
                            play("square_negative" * amp(a * cr_cancel_amp), cr_cancel, duration=t)
                            # pi pulse on control
                            align(qc_xy, cr_drive, cr_cancel)
                            play("x180", qc_xy)
                            # align for the next step and clear the phase shift
    ...
    with stream_processing():
        ...
        for ind, rr in enumerate(resonators):
            I_st[ind].buffer(2).buffer(3).buffer(len(ts_cycles)).buffer(len(amps)).average().save(f"I_{rr}")
            Q_st[ind].buffer(2).buffer(3).buffer(len(ts_cycles)).buffer(len(amps)).average().save(f"Q_{rr}")
            state_st[ind].boolean_to_int().buffer(2).buffer(3).buffer(len(ts_cycles)).buffer(len(amps)).average().save(f"state_{rr}")        
...
if __name__ == "__main__":
    ...
    if simulate:
        ...
        try:
            ...
            data_handler.save_data(data=save_data_dict, name="cr_calib_ham_tomo_direct_vs_amp")
