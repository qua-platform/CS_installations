...
with program() as parity_meas:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(*from_array(tau, taus)):
            ...
            state1, I1, Q1 = macros.readout_macro(
                threshold=ge_threshold, state=state1, I=I1, Q=Q1
            )
            ...
            align("qubit", "resonator")
            state2, I2, Q2 = macros.readout_macro(
                threshold=ge_threshold, state=state2, I=I2, Q=Q2
            )
    ...
    with stream_processing():
        ...
        (
            state1_st.boolean_to_int().buffer(len(taus))
            - state2_st.boolean_to_int().buffer(len(taus))
        ).average().save("state")
        n_st.save("iteration")
