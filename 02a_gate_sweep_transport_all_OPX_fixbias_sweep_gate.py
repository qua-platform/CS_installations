...
gate_dc_offsets = np.arange(-0.35, 0.03, 0.0025)
...
with program() as gate_sweep_transport:
    ...
    set_dc_offset('drain_tia', 'single', 0.2)

    with for_(*from_array(dc, gate_dc_offsets)):

        set_dc_offset('right_plunger', 'single', dc)
...
if simulate:
    ...
    while results.is_processing():
        ...
        plt.cla()
        plt.plot(gate_dc_offsets[:len(i_drain_pA)], -1 * (i_drain_pA - background), '.-')
