...
for c, t in qubit_pairs:
    ...
    LO_cr_cancel = CR_CANCEL_CONSTANTS[f"cr_cancel_c{c}t{t}"]["LO"]
    assert LO_cr_drive + IF_cr_drive == LO_cr_cancel + IF_cr_cancel, f"The frequencies are not matching between cr_drive_c{c}t{t} and cr_cancel_c{c}t{t}!"
...
for c, t in qubit_pairs:
    ...
    LO_zz_target = ZZ_TARGET_CONSTANTS[f"zz_target_c{c}t{t}"]["LO"]
    assert LO_zz_control + IF_zz_control == LO_zz_target + IF_zz_target, f"The frequencies are not matching between zz_control_c{c}t{t} and zz_target_c{c}t{t}!"
