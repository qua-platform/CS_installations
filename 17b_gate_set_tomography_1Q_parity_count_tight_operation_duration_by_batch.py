...
if simulate:
    ...
    def get_print_this_log(n_shots, i_row, start_time, seq_len, C, P, G, d, M, D):
        current_datetime = datetime.now()
        current_datetime_str = current_datetime.strftime("%Y/%m/%d-%H:%M:%S")
        elapsed_time = current_datetime - start_time
        elapsed_time_secs = int(elapsed_time.total_seconds())
        _this_log = f"{current_datetime_str}, n_shots: {n_shots}, no: {i_row + 1}, seq_len: {seq_len}, circ_idx={C}, seq=(P={P}, G={G}, d={d}, M={M}), D={D} elapsed_secs: {elapsed_time_secs}"
        print(_this_log)
        return _this_log
    ...
    for _n_shots in list_n_shots:
        ...
        for i_row, (_, row) in enumerate(this_df.iterrows()):
            ...
            # Get log
            _this_log = get_print_this_log(n_shots=_n_shots, i_row=i_row, start_time=start_time, seq_len=seq_len, C=C, P=P, G=G, d=d, M=M, D=D)
