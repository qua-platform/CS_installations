...
def get_print_this_log(n_shots, i_batch, i_row, start_time, seq_len, C, P, G, d, M, Ph4, Ph):
    ...
    _this_log = f"{current_datetime_str}, n_shots: {n_shots}, batch: {i_batch}, no: {i_row + 1}, seq_len: {seq_len}, circ_idx={C}, seq=(P={P}, G={G}, d={d}, M={M}), Ph4={Ph4}, Ph={Ph}, elapsed_secs: {elapsed_time_secs}"
    print(_this_log)
    return _this_log
...
with program() as PROGRAM_GST:
    ...
    division_count = declare(int)
    division_count_st = declare_stream()
    ...
    with for_(circ, 0, circ < num_cicuits, circ + 1):
        ...
        assign(division_count, division * batch_size - 1)
        save(division_count, division_count_st)
        with if_(circ == division_count):
    ...
    with stream_processing():
        ...
        division_count_st.save_all("division_count")
...
if simulate:
    ...
    fetch_names = ["circs", "division_count", "circ_idx_history", f"P0_count_{tank_circuit}", f"P1_count_{tank_circuit}"]
    ...
    for i_row, (_, row) in enumerate(this_df.iterrows()):
        ...
        if job.is_paused():
            ...
            # Data to save
            print("save result!")
            np.savez(file=data_handler.path / f"data_n-shots={n_shots:06d}_batch={i_batch:06d}.npz", **data_dict)
            i_batch += 1
            job.resume()

        # Get log
        _this_log = get_print_this_log(
            n_shots=n_shots, i_batch=i_batch, i_row=i_row,
            start_time=start_time, seq_len=seq_len,
            C=C, P=P, G=G, d=d, M=M, Ph4=Ph4, Ph=Ph,
        )
        with open(data_handler.path / "log.txt", encoding="utf8", mode="a") as f:
            f.write(_this_log.replace("_", "") + "\n")  # Append the log message to the file

    results = fetching_tool(job, data_list=["circs", "division_count"])
    res = results.fetch_all()
