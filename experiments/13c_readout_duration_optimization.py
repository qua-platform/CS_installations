...
with program() as ro_weights_opt:
    ...
    with stream_processing():
        ...
        for q in range(2):
            # all shots
            Ig_st[q].buffer(number_of_divisions).buffer(n_avg).save(f"Ig{q}")
            Qg_st[q].buffer(number_of_divisions).buffer(n_avg).save(f"Qg{q}")
            Ie_st[q].buffer(number_of_divisions).buffer(n_avg).save(f"Ie{q}")
            Qe_st[q].buffer(number_of_divisions).buffer(n_avg).save(f"Qe{q}")
