...
durations = np.arange(40, 200, 4) # np.arange(16, 200, 4)
        ...
        with for_(*from_array(f, frequencies)):  # Loop over the qubit pulse amplitude
            ...
            with for_(*from_array(t, durations)):  # Loop over the qubit pulse duration
                with strict_timing_():  # Ensure that the sequence will be played without gap
                    ...
                    seq.add_step(voltage_point_name="manipulation", duration=t, ramp_duration=ramp_duration_manip)
                    ...
                    wait((duration_init + duration_interim + ramp_duration_manip) * u.ns , "qubit")
                    ...
                    # wait((duration_init + duration_interim + ramp_duration_manip) * u.ns + (t >> 2), "tank_circuit")
                    wait((duration_init + duration_interim + ramp_duration_manip) * u.ns + (t >> 2), "tank_circuit")
