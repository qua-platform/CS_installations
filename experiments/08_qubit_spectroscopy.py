...
duration_manip = cw_len + delay_before_readout
ramp_duration_manip = 100 # for adiabatic evolution
...
seq.add_points("intermediate", level_interim, duration_interim)
            ...
            with for_(n, 0, n < n_avg, n + 1):  # The averaging loop
                with for_(*from_array(f, IFs)):  # Loop over the qubit pulse amplitude
                    ...
                    with strict_timing_():  # Ensure that the sequence will be played without gap
                        ...
                        seq.add_step(voltage_point_name="intermediate")
                        seq.add_step(voltage_point_name="manipulation", ramp_duration=ramp_duration_manip)
                        ...
                        wait((duration_init + duration_interim + ramp_duration_manip + duration_manip - delay_before_readout - cw_len) * u.ns, "qubit")
                        play("cw", "qubit")

                        # Measure the dot right after the qubit manipulation
                        wait((duration_init + duration_interim + ramp_duration_manip + duration_manip) * u.ns, "tank_circuit")
...
if simulate:
    ...
    plt.figure()
    plt.subplot(211)
    plt.axhline(level_init[0], color="k", alpha=0.3, linestyle="--")
    plt.axhline(level_interim[0], color="k", alpha=0.3, linestyle="--")
    plt.axhline(level_manip[0], color="k", alpha=0.3, linestyle="--")
    plt.axhline(level_readout[0], color="k", alpha=0.3, linestyle="--")
    plt.axhline(level_init[1], color="k", alpha=0.3, linestyle="--")
    plt.axhline(level_interim[1], color="k", alpha=0.3, linestyle="--")
    plt.axhline(level_manip[1], color="k", alpha=0.3, linestyle="--")
    plt.axhline(level_readout[1], color="k", alpha=0.3, linestyle="--")
    plt.yticks(
        [
            level_readout[1],
            level_manip[1],
        ...
        ["readout_2", "manip_2", "interim_2", "init_2", "0", "init_1", "interim_1", "manip_1", "readout_1"],
    )
