...
with program() as raw_trace_prog:
    ...
    adc_st = declare_stream(adc_trace=True)  # The stream to store the raw ADC trace for the RF line
...
if simulate:
    ...
    plt.plot(adc1_single_run, label="RF input")
    plt.xlabel("Time [ns]")
    ...
    plt.plot(adc1, label="RF input")
    plt.xlabel("Time [ns]")
    plt.legend()
    plt.tight_layout()

    print(f"RF input mean: {np.mean(adc1)} V\n")
