...
def play_T1(I, I_st, t: int, start_from_one: bool = True, measure_in_one: bool = True, mw_amp_ratio: float = 1):
    play("x180" * amp(int(start_from_one) * mw_amp_ratio), "NVs")
    wait(t, "NVs")  # variable delay in spin Echo
    play("x180" * amp(int(measure_in_one) * mw_amp_ratio), "NVs")  # Pi pulse to qubit to measure second state
