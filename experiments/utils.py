...
def save_files(files: List[Path], dir_data: Path, base_dir: Path) -> None:
    ...
    for file_from in files:
        if file_from.is_file():
            ...
            qm_log.info(f"saved: {str(file_from)}")
        else:
            qm_log.warning(f"not found: {str(file_from)}")
...
def save_files_and_get_dir_data(
    ...
    :param basedir_data: str: Base directory to save data and files.
    :param script_path: str: Path of current script.
    :param config_files: bool: Configuration related files to save
    """
