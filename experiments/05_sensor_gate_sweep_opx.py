...
offset_to = +0.2
n_points = 101
d_offset = (offset_to - offset_from) / (n_points - 1)
offsets = np.linspace(offset_from, offset_to, n_points)
...
with program() as charge_sensor_sweep:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        ...
        play("step" * amp(offsets[0] / sensor_step_amp), "sensor_gate_sticky")
        ...
        with for_(i, 0, i < n_points, i + 1):
            with if_(i > 0):
                play("step" * amp(d_offset / sensor_step_amp), "sensor_gate_sticky")
                # Wait for the voltage to settle (depends on the bias-tee cut-off frequency)
                wait(1 * u.ms, "sensor_gate_sticky")
