...
class Parameters(NodeParameters):
    ...
    flux_detuning_in_mhz: float = 1.0
    min_wait_time_in_ns: int = 2250
    max_wait_time_in_ns: int = 3001
    wait_time_step_in_ns: int = 250
...
virtual_detuning = int(1e6 * node.parameters.frequency_detuning_in_mhz)
flux_point = node.parameters.flux_point_joint_or_independent  # 'independent' or 'joint'
flux_detunings = (np.array([-1,0,1])*node.parameters.flux_detuning_in_mhz).astype(int)
# flux_detunings = [0]
# idle_times = [100]
...
with program() as ramsey:
    ...
    state = declare(int)
    state_av = [declare(fixed) for _ in range(len(idle_times)*len(flux_detunings))]
    state_st_av = [declare_stream() for _ in range(len(idle_times)*len(flux_detunings))]
    ...
    with for_(n, 0, n < n_avg, n + 1):
        save(n, n_st)     
        # with for_(*from_array(detuning, flux_detunings)): 
        for i_d, detuning in enumerate(flux_detunings):
            # assign(flux, -1/qubit.freq_vs_flux_01_quad_term*1e-6)
            # assign(flux, Math.sqrt(flux * detuning) )   
            flux = np.sqrt(-1e6 * detuning / qubit.freq_vs_flux_01_quad_term)                          
            # with for_(*from_array(t, idle_times)):
            for i_t, t in enumerate(idle_times):
                readout_state(qubit, init_state)
                qubit.align()
                # assign(phi, Cast.mul_fixed_by_int(virtual_detuning * 1e-9, 4 * t ))
                assign(phi, virtual_detuning * 1e-9 * 4 * t)
                qubit.align()
                ...
                assign(state, init_state ^ state)
                # save(state[i], state_st[i])
                j = i_d*len(idle_times)+i_t
                assign(state_av[j], state_av[j] + Cast.to_fixed(state) / n_avg)
                # Reset the frame of the qubits in order not to accumulate rotations
                reset_frame(qubit.xy.name)
                qubit.align()
                save(state_av[j], state_st_av[j])
    align()

# with stream_processing():
#     n_st.save("n")
#     for i in range(len(idle_times)*len(flux_detunings)):
#         state_st_av[i].save(f"state{i + 1}")
