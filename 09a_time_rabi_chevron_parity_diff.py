...
frequencies = np.arange(-1 * u.MHz, 1.1 * u.MHz, 0.1 * u.MHz) + QUBIT_CONSTANTS[qubit]["IF"]
    ...
    with for_(n, 0, n < n_avg, n + 1):
        ...
        with for_(*from_array(f, frequencies)):  # Loop over the qubit pulse amplitude
            ...
            with for_(*from_array(d, durations)):  # Loop over the qubit pulse duration
                assign(dwell, (RF_SWITCH_DELAY + d + RF_SWITCH_DELAY) >> 2)
...
if simulate:
    ...
    while results.is_processing():
        ...
        plt.pcolor(durations, frequencies / u.MHz, P_diff_avg)
